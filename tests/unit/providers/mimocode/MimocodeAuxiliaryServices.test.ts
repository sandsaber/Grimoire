...
import { MimocodeInlineEditService } from '@/providers/mimocode/auxiliary/MimocodeInlineEditService';
import { MimocodeInstructionRefineService } from '@/providers/mimocode/auxiliary/MimocodeInstructionRefineService';
import { MimocodeTitleGenerationService } from '@/providers/mimocode/auxiliary/MimocodeTitleGenerationService';
 ...
 * Both were carried over from `MimocodeAuxQueryRunner`, and both are invisible
 * until something counts them.
 */
describe('MiMoCode auxiliary services', () => {
  function createPlugin(): {
    ...
    return {
      ...
      plugin: {
        ...
        getMimocodeExecution: () => ({ createAuxRunner }),
      },
    };
  }
  ...
  it('asks for nothing until something is asked of it', () => {
    ...
    new MimocodeInlineEditService(harness.plugin);
    new MimocodeInstructionRefineService(harness.plugin);
    new MimocodeTitleGenerationService(harness.plugin);
  ...
  it('keeps one conversation for an inline edit and its follow-up', async () => {
    const harness = createPlugin();
    const service = new MimocodeInlineEditService(harness.plugin);
  ...
  it('refines instructions in a conversation of its own', async () => {
    const harness = createPlugin();
    const service = new MimocodeInstructionRefineService(harness.plugin);
  ...
  it('generates each title on a conversation of its own, and ends it', async () => {
    const harness = createPlugin();
    const service = new MimocodeTitleGenerationService(harness.plugin);
