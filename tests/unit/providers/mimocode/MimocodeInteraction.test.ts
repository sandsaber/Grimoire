...
/**
 * What a flipped MiMoCode tab asks before it edits anything, and how the answer
 * gets back.
 *
 * The bridge and the presenter are the shared ACP ones, proven by OpenCode and
 * Grok; what is MiMoCode's is the sentence a person reads and the fact that
 ...
 */
describe('MiMoCode interactions', () => {
  ...
  async function open(
    bridge: MimocodeInteractionBridge,
  ...
  function createPresenter(callbacks: ExecutionInteractionCallbacks): {
    bridge: MimocodeInteractionBridge;
    presenter: MimocodeInteractionPresenter;
  } {
    const bridge = new MimocodeInteractionBridge();
    return { bridge, presenter: new MimocodeInteractionPresenter(bridge, () => callbacks) };
  }

  it('describes the permission in MiMoCode own words', async () => {
    const bridge = new MimocodeInteractionBridge();
  ...
  it('asks the surface in the words the request was described with', async () => {
    ...
    expect(approval).toHaveBeenCalledWith(
      'bash',
      { command: 'ls' },
      'MiMoCode wants to run a shell command.',
