...
/**
 * The auxiliary half of MiMoCode's reference store, and the runner that feeds it.
 *
 ...
 */
describe('MiMoCode auxiliary requests', () => {
  function createStore(
    environment: Partial<MimocodeAuxiliaryEnvironment> = {},
    observed: MimocodeAuxiliaryRequest[] = [],
  ): { requests: MimocodeExecutionRequests; seen: MimocodeAuxiliaryRequest[] } {
    let next = 0;
    const requests = new MimocodeExecutionRequests(
      () => `ref-${(next += 1)}`,
      ...
      async request => {
        ...
        return {
          executable: '/usr/bin/mimocode',
          cwd: '/vault',
          environment: { MIMOCODE_CONFIG: '/vault/.grimoire/mimocode/auxiliary/title-gen/config.json' },
  ...
  const titleRequest: MimocodeAuxiliaryRequest = {
    purpose: 'title-gen',
  ...
  it('resolves an auxiliary turn into a launch, a conversation and a session to configure', async () => {
    ...
    expect(invocation).toEqual({
      ...
      mcpServers: [],
      retentionKey: 'mimocode-auxiliary:title-gen:aux-1',
    ...
    await expect(requests.resolveLaunch('ref-2')).resolves.toEqual({
      ...
      cwd: '/vault',
      environment: {
        MIMOCODE_CONFIG: '/vault/.grimoire/mimocode/auxiliary/title-gen/config.json',
      },
    });
  });
  ...
  it('applies the model per turn, and only when there is one to apply', async () => {
    ...
    const configured = await withModel.requests.resolveAuxiliary(
      withModel.requests.referenceAuxiliary({ ...titleRequest, model: 'mimocode:anthropic:sonnet' }),
    );
  ...
  it('changes the restart fingerprint when the instructions change', async () => {
    ...
    // MiMoCode reads its system prompt from a file the artifacts write, so a
  ...
  it('refuses a reference it did not mint, and mints each one once', async () => {
    ...
    // for.
    await expect(requests.resolveAuxiliary(requestRef)).rejects.toThrow('Unknown MiMoCode auxiliary');
    await expect(requests.resolveAuxiliary('ref-borrowed')).rejects.toThrow('Unknown MiMoCode auxiliary');
  });
  ...
  describe('the runner the auxiliary services keep calling', () => {
    ...
    it('passes the caller its cancellation and its stream', async () => {
      ...
      const answer = await harness.runner.query({
        abortController,
        model: 'mimocode:anthropic:sonnet',
        systemPrompt: 'Name the conversation.',
      ...
      expect(harness.seen[0].config.model).toBe('mimocode:anthropic:sonnet');
    });
