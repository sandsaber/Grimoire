...
const OWNER = { kind: 'conversation' as const, ownerId: 'mimocode-lifecycle' };

describe('MiMoCode execution lifecycle integration', () => {
  it('persists one stable native run identity through result and terminal ingestion', async () => {
    ...
    const backend = new MimocodeExecutionBackend({
      ...
      resultSink: {
        storeResult: async () => ({
          kind: 'committed',
          result: { resultId: 'mimocode-result', storage: 'projection' },
        }),
      },
      emptyResultPolicy: { resolve: async () => ({ kind: 'no-provider-error' }) },
    ...
    expect(registry.getRun(RUN_ID)).toMatchObject({
      ...
      resultRef: { resultId: 'mimocode-result', storage: 'projection' },
...
async function waitFor(predicate: () => boolean): Promise<void> {
  ...
  throw new Error('Timed out waiting for MiMoCode dispatch.');
}
