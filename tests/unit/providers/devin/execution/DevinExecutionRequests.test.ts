...
/**
 * The three reference spaces one Devin turn passes through.
 *
 ...
 */
describe('DevinExecutionRequests', () => {
  const prompt: readonly AcpContentBlock[] = [{ type: 'text', text: 'Reply with exactly: ok' }];

  function createStore(overrides: Partial<DevinInvocationEnvironment> = {}): {
    store: DevinExecutionRequests;
    launches: number;
  } {
    let launches = 0;
    let next = 0;
    const store = new DevinExecutionRequests(
      () => `ref-${++next}`,
      async () => {
        launches += 1;
        return {
  ...
  it('resolves a turn into the launch and the prompt the backend dispatches', async () => {
    ...
    const requestRef = store.reference({
      prompt,
      messageId: 'message-1',
      dynamic: { modeId: 'plan', modelId: 'swe-1-6-slow' },
    });
    ...
    await expect(store.resolveLaunch(invocation.startupRef)).resolves.toEqual({
      executable: '/usr/local/bin/devin',
      // A subcommand: the recorded handshake is `devin acp`.
      arguments: ['acp'],
      cwd: '/vault',
      environment: { DEVIN_MODEL: 'opus' },
    });
  ...
  it('holds a prompt no longer than the dispatch that needed it', async () => {
    ...
    await expect(store.resolve(requestRef)).rejects.toThrow('Unknown Devin request reference.');
  });
  ...
  it('refuses a reference it never minted rather than guessing', async () => {
    ...
    await expect(store.resolve('ref-nonsense'))
      .rejects.toThrow('Unknown Devin request reference.');
    await expect(store.resolveLaunch('ref-nonsense'))
      .rejects.toThrow('Unknown Devin startup reference.');
    await expect(store.resolveDynamic('ref-nonsense'))
      .rejects.toThrow('Unknown Devin dynamic configuration reference.');
  });
  ...
  it('stays bounded when turns are composed and never dispatched', () => {
    ...
    return expect(store.resolve(first)).rejects.toThrow('Unknown Devin request reference.');
  });
  ...
  it('drops everything it is holding when the composition goes away', async () => {
    ...
    await expect(store.resolve(requestRef)).rejects.toThrow('Unknown Devin request reference.');
    await expect(store.resolveLaunch(startupRef)).rejects.toThrow('Unknown Devin startup reference.');
  });
});
