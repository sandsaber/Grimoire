...
/**
 * What one Devin turn says, and what the flip must not have changed about it.
 *
 * These assertions lived inside `DevinChatRuntime.test.ts` and were driving a
...
describe('buildDevinPrompt', () => {
  it('carries every piece of the vault a turn was composed with', () => {
    const prompt = buildDevinPromptText({
      browserSelection: {
  ...
  it('rebuilds prior conversation context when a turn carries history', () => {
    ...
    const prompt = buildDevinPromptText({ text: 'Apply that to DoorTextStyle.' }, history);
  ...
  it('takes the flag from the request as well as from the turn options', () => {
    ...
    const fromRequest = buildDevinPromptBlocks({ text: 'Plan', orchestratorMode: true });
