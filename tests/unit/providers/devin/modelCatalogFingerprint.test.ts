...
import { buildDevinModelCatalogFingerprint } from '@/providers/devin/modelCatalogFingerprint';
import { getDevinProviderSettings } from '@/providers/devin/settings';
...
describe('buildDevinModelCatalogFingerprint', () => {
  ...
  it('changes when the CLI is upgraded in place', () => {
    ...
    const before = buildDevinModelCatalogFingerprint(settings, '/usr/local/bin/devin', '');
    ...
    const after = buildDevinModelCatalogFingerprint(settings, '/usr/local/bin/devin', '');

    expect(mockedFingerprint).toHaveBeenCalledWith('/usr/local/bin/devin');
  ...
  it('stays stable while the same build sits at the same path', () => {
    ...
    expect(buildDevinModelCatalogFingerprint(settings, '/usr/local/bin/devin', ''))
      .toBe(buildDevinModelCatalogFingerprint(settings, '/usr/local/bin/devin', ''));
  });
});
