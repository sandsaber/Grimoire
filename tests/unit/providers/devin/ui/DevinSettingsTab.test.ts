...
import { devinSettingsTabRenderer } from '@/providers/devin/ui/DevinSettingsTab';
...
jest.mock('@/providers/devin/app/DevinWorkspaceServices', () => ({
  maybeGetDevinWorkspaceServices: jest.fn(() => ({
    commandCatalog: null,
    modelCatalog: {
      isAvailable: () => true,
...
function createContext(discoveredModels: unknown[]) {
  const plugin = {
    ...
    settings: {
      providerConfigs: {
        devin: { enabled: true, discoveredModels },
      },
    },
  } as any;
...
function renderTab(discoveredModels: unknown[]) {
  ...
  devinSettingsTabRenderer.render(createMockEl(), context);
  onClick.mockRestore();

  return { context, handlers, plugin, setButtonText, setDisabled };
}

describe('DevinSettingsTab model refresh', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('lets the user ask the CLI for its current model list', async () => {
    ...
    expect(getDevinProviderSettings(plugin.settings).discoveredModels).toHaveLength(2);
