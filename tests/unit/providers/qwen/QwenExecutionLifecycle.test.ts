...
const OWNER = { kind: 'conversation' as const, ownerId: 'qwen-lifecycle' };

describe('Qwen Code execution lifecycle integration', () => {
  it('persists one stable native run identity through result and terminal ingestion', async () => {
    ...
    const backend = new QwenExecutionBackend({
      ...
      resultSink: {
        storeResult: async () => ({
          kind: 'committed',
          result: { resultId: 'qwen-result', storage: 'projection' },
        }),
      },
    ...
    expect(registry.getRun(RUN_ID)).toMatchObject({
      ...
      resultRef: { resultId: 'qwen-result', storage: 'projection' },
...
async function waitFor(predicate: () => boolean): Promise<void> {
  ...
  throw new Error('Timed out waiting for Qwen Code dispatch.');
}
