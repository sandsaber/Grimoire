...
import {
  ...
  type QwenExecutionInvocation,
  type QwenExecutionScheduler,
  type QwenPreparedInteraction,
} from '@/providers/qwen/execution/QwenExecutionBackend';

describe('QwenExecutionBackend', () => {
  it('initializes a managed client, creates a native session, and commits assistant output', async () => {
    ...
    fixture.client.emit(agentText('native-session', 'Qwen Code result'));
    ...
    expect(fixture.stored).toEqual(['Qwen Code result']);
  ...
  it('loads the exact saved ACP session before dispatch', async () => {
    ...
    expect([
      ...
      `session/prompt:${fixture.client.promptRequests[0].messageId}`,
    ]).toEqual(trace.cases.resumeLoadWithoutResponseIdentity);
  });
  ...
  it('restarts only for startup configuration changes and reloads the native session', async () => {
    ...
    const fixture = createFixture({
      ...
      requestResolve: async requestRef => requestRef === 'first'
        ? invocation()
        : { ...invocation(), restartFingerprint: 'qwen-v2', messageId: 'message-2' },
    });
    const session = await createSession(fixture.backend);
  ...
  it('persists a structured Qwen question as a question interaction', async () => {
    const resolve = jest.fn(async (responseId: string) => ({
      answers: { '0': 'Content' },
      outcome: { outcome: 'selected' as const, optionId: responseId },
    }));
    const fixture = createFixture({
      interactionPrepare: async () => ({
        kind: 'question',
        presentationRef: 'question-presentation',
        responseIds: ['submit', 'cancel'],
  ...
  it('projects active commands and usage while keeping opaque child output out of the result', async () => {
    ...
    fixture.client.emit(opaqueAgentText('native-session', 'private child output'));
    fixture.client.emit(opaqueAgentToolAtRoot('native-session'));
    ...
    expect(summarizeEvents(captured)).toEqual(trace.eventCases.opaqueAgentEvidence);
    expect(fixture.commandLog[0]).toBe(trace.cases.activeSessionCommands[0]);
    expect([
      fixture.usageLog[0],
      `context:${fixture.client.extensionRequests[0]}`,
      fixture.usageLog[1],
    ]).toEqual(trace.cases.usageContext);

    await fixture.backend.dispose();
    expect(fixture.commandLog.at(-1)).toBe(trace.cases.activeSessionCommands[1]);
  });

  it('clears active-session commands even when optional usage cleanup fails', async () => {
    const fixture = createFixture({
    ...
    expect(fixture.commandLog).toEqual(trace.cases.activeSessionCommands);
  });

  it('cancels a permission prepared after its bounded request already failed closed', async () => {
    const preparation = deferred<QwenPreparedInteraction>();
...
function createFixture(options: {
  ...
  readonly dynamicApply?: QwenExecutionBackendContext['dynamicApplier']['apply'];
  readonly interactionPrepare?: () => Promise<QwenPreparedInteraction>;
  readonly usageDetach?: () => void;
  readonly requestResolve?: (requestRef: string) => Promise<QwenExecutionInvocation>;
} = {}) {
  ...
  const backend = new QwenExecutionBackend({
    ...
    commands: {
      replace: (sessionId, commands) => {
        commandLog.push(`commands:${sessionId}:${commands.map(command => command.name).join(',')}`);
      },
      clear: sessionId => { commandLog.push(`commands:${sessionId}:clear`); },
    },
    usage: {
      attach: () => undefined,
...
class FakeManagedAcpClient implements ManagedAcpClient {
  ...
  async requestExtension(method: string): Promise<unknown> {
    this.extensionRequests.push(method);
    return { usage: { contextWindowSize: 1_000_000, totalTokens: 250_000 } };
  }
...
class FakeScheduler implements QwenExecutionScheduler {
...
function invocation(): QwenExecutionInvocation {
  return {
    startupRef: 'qwen-startup',
    restartFingerprint: 'qwen-v1',
    cwd: '/vault',
...
function request(suffix: string, requestRef = 'request-ref'): ExecutionRequest {
  return {
    ...
    owner: { kind: 'conversation', ownerId: 'qwen-tests' },
    resultExpectation: 'required',
    requestRef,
  };
}

async function createSession(backend: QwenExecutionBackend, nativeSessionRef?: string) {
...
function opaqueAgentText(sessionId: string, text: string): AcpSessionNotification {
  return {
    sessionId,
    update: {
      _meta: { parentToolCallId: 'agent-call-1', subagentType: 'Explore' },
      sessionUpdate: 'agent_message_chunk',
      content: { type: 'text', text },
      messageId: 'nested-assistant',
    } as unknown as AcpSessionNotification['update'],
  };
}

function opaqueAgentToolAtRoot(sessionId: string): AcpSessionNotification {
  return {
    sessionId,
    update: {
