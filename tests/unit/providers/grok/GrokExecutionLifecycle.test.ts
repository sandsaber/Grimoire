...
const OWNER = { kind: 'conversation' as const, ownerId: 'grok-lifecycle' };

describe('Grok Build execution lifecycle integration', () => {
  it('persists one stable native run identity through result and terminal ingestion', async () => {
    ...
    const backend = new GrokExecutionBackend({
      ...
      interactionBridge: {
        prepareApproval: async () => { throw new Error('No interaction.'); },
        prepareQuestion: async () => { throw new Error('No interaction.'); },
      },
      resultSink: {
        storeResult: async () => ({
          kind: 'committed',
          result: { resultId: 'grok-result', storage: 'projection' },
        }),
      },
      usage: {
        attach: () => undefined,
    ...
    expect(registry.getRun(RUN_ID)).toMatchObject({
      ...
      resultRef: { resultId: 'grok-result', storage: 'projection' },
...
async function waitFor(predicate: () => boolean): Promise<void> {
  ...
  throw new Error('Timed out waiting for Grok Build dispatch.');
}
