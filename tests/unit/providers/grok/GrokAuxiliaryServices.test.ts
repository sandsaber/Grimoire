 ...
 * the permission mode a turn is launched under and the artifacts it runs in; the
 * number of runners decides whether a second message reaches the first one's
 * session. Both were carried over from `GrokAuxQueryRunner`, and both are
...
describe('Grok auxiliary services', () => {
  function createPlugin(): {
    ...
    return {
      ...
      plugin: {
        ...
        getGrokExecution: () => ({ createAuxRunner }),
      },
    };
  }
  ...
  it('asks for nothing until something is asked of it', () => {
    ...
    new GrokInlineEditService(harness.plugin);
    new GrokInstructionRefineService(harness.plugin);
    new GrokTitleGenerationService(harness.plugin);
  ...
  it('keeps one conversation for an inline edit and its follow-up', async () => {
    const harness = createPlugin();
    const service = new GrokInlineEditService(harness.plugin);
    ...
    // The property the whole retained session exists for: the second message has
    // to reach the session the first one made. Two runners would be two
  ...
  it('refines instructions in a conversation of its own', async () => {
    const harness = createPlugin();
    const service = new GrokInstructionRefineService(harness.plugin);

    await service.refineInstruction('do the thing', 'existing instructions');

    // Its own purpose, which is its own managed home and its own permission
    // mode: a refinement must not run under the instructions an inline edit was
    // launched with, and must not be launched able to read.
    expect(harness.calls).toEqual(['instructions']);
  });

  it('generates each title on a conversation of its own, and ends it', async () => {
    const harness = createPlugin();
    const service = new GrokTitleGenerationService(harness.plugin);
