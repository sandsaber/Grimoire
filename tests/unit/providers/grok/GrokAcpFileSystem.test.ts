...
import { GrokAcpFileSystem } from '@/providers/grok/execution/GrokAcpFileSystem';

describe('GrokAcpFileSystem', () => {
  it('contains reads and approved writes to the session workspace on every host path style', async () => {
    const root = await mkdtemp(join(tmpdir(), 'grimoire-grok-fs-'));
    const approvals: string[] = [];
    const operations: string[] = [];
    const fileSystem = new GrokAcpFileSystem({
  ...
  it('rejects workspace escape before approval or filesystem access', async () => {
    const root = await mkdtemp(join(tmpdir(), 'grimoire-grok-contained-'));
