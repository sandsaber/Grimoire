...
import {
  ...
  type KimicodeExecutionInvocation,
  type KimicodeExecutionScheduler,
  type KimicodePreparedInteraction,
} from '@/providers/kimicode/execution/KimicodeExecutionBackend';

describe('KimicodeExecutionBackend', () => {
  it('initializes a managed client, creates a native session, and commits assistant output', async () => {
    ...
    fixture.client.emit(agentText('native-session', 'Kimi Code result'));
    ...
    expect(fixture.stored).toEqual(['Kimi Code result']);
  ...
  it('restarts only for startup configuration changes and reloads the native session', async () => {
    ...
    const fixture = createFixture({
      ...
      requestResolve: async requestRef => requestRef === 'first'
        ? invocation()
        : { ...invocation(), restartFingerprint: 'kimicode-v2', messageId: 'message-2' },
    });
    const session = await createSession(fixture.backend);
  ...
  it('cancels a permission prepared after its bounded request already failed closed', async () => {
    const preparation = deferred<KimicodePreparedInteraction>();
...
function createFixture(options: {
  ...
  readonly interactionPrepare?: () => Promise<KimicodePreparedInteraction>;
  readonly auxiliaryExecute?: (requestRef: string, signal: AbortSignal) => Promise<string>;
  readonly requestResolve?: (requestRef: string) => Promise<KimicodeExecutionInvocation>;
} = {}) {
  ...
  const backend = new KimicodeExecutionBackend({
...
class FakeScheduler implements KimicodeExecutionScheduler {
...
function invocation(): KimicodeExecutionInvocation {
  return {
    startupRef: 'kimicode-startup',
    restartFingerprint: 'kimicode-v1',
    cwd: '/vault',
...
function request(suffix: string, requestRef = 'request-ref'): ExecutionRequest {
  return {
    ...
    owner: { kind: 'conversation', ownerId: 'kimicode-tests' },
    resultExpectation: 'required',
    requestRef,
  };
}

async function createSession(backend: KimicodeExecutionBackend, nativeSessionRef?: string) {
