...
describe('Kimi Code execution composition', () => {
  ...
  describe('auxiliary work, on the kernel', () => {
    ...
    it('writes an agent that cannot write, and gives inline edit one that can read', async () => {
      ...
      // set at all: Kimi Code has no per-request permission mode, so what a turn
      ...
      const agentOf = (purpose: string, id: string): Record<string, any> => JSON.parse(
        readFileSync(join(vault, '.grimoire', 'kimicode', 'auxiliary', purpose, 'config.json'), 'utf8'),
      ).agent[id];
      ...
      expect(existsSync(join(vault, '.grimoire', 'kimicode', 'auxiliary', 'inline', 'system.md')))
        .toBe(true);
      execution.dispose();
      await host.dispose();
    });
    ...
    it('launches kimicode from PATH when no CLI path is configured', async () => {
      const plugin = createPlugin();
      ...
      const launch = await execution.turnRequests.resolveLaunch(startupRefs[0]);
      expect(launch.executable).toBe('kimicode');
      expect(launch.arguments).toEqual(['acp']);
      execution.dispose();
      await host.dispose();
    });
    ...
    it('applies the model the caller chose, decoding the id space it came in', async () => {
      ...
      await execution.createAuxRunner('title-gen').query({
        systemPrompt: 'Name the conversation.',
        model: 'kimicode:big-pickle-provider:big-pickle',
      }, 'the message');
    ...
    it('falls back to the model the chat is set to when the caller names none', async () => {
      const plugin = createPlugin();
      plugin.settings.savedProviderModel = { kimicode: 'kimicode:big-pickle-provider/big-pickle' };
      updateKimicodeProviderSettings(plugin.settings, {
