...
/**
 * What a Kimi Code turn commits, and what D2 forbids it from committing.
 *
 * The answer is already durable in the conversation and in Kimi Code's own
 ...
 */
describe('KimicodeProjectionResultSink', () => {
  const commit = (overrides: Partial<{
    output: string;
    nativeSessionRef: string;
    nativeRunRef: string;
    signal: AbortSignal;
  }> = {}) => new KimicodeProjectionResultSink().storeResult({
    output: 'the answer',
  ...
  it('still names a turn the provider gave no id for', async () => {
    const outcome = await new KimicodeProjectionResultSink().storeResult({
      output: 'the answer',
      nativeSessionRef: 'ses_abc',
      signal: new AbortController().signal,
    });
