...
const OWNER = { kind: 'conversation' as const, ownerId: 'kimicode-lifecycle' };

describe('Kimi Code execution lifecycle integration', () => {
  it('persists one stable native run identity through result and terminal ingestion', async () => {
    ...
    const backend = new KimicodeExecutionBackend({
      ...
      resultSink: {
        storeResult: async () => ({
          kind: 'committed',
          result: { resultId: 'kimicode-result', storage: 'projection' },
        }),
      },
    ...
    expect(registry.getRun(RUN_ID)).toMatchObject({
      ...
      resultRef: { resultId: 'kimicode-result', storage: 'projection' },
...
async function waitFor(predicate: () => boolean): Promise<void> {
  ...
  throw new Error('Timed out waiting for Kimi Code dispatch.');
}
