...
import { KimicodeInlineEditService } from '@/providers/kimicode/auxiliary/KimicodeInlineEditService';
import { KimicodeInstructionRefineService } from '@/providers/kimicode/auxiliary/KimicodeInstructionRefineService';
import { KimicodeTitleGenerationService } from '@/providers/kimicode/auxiliary/KimicodeTitleGenerationService';
 ...
 * Both were carried over from `KimicodeAuxQueryRunner`, and both are invisible
 * until something counts them.
 */
describe('Kimi Code auxiliary services', () => {
  function createPlugin(): {
    ...
    return {
      ...
      plugin: {
        ...
        getKimicodeExecution: () => ({ createAuxRunner }),
      },
    };
  }
  ...
  it('asks for nothing until something is asked of it', () => {
    ...
    new KimicodeInlineEditService(harness.plugin);
    new KimicodeInstructionRefineService(harness.plugin);
    new KimicodeTitleGenerationService(harness.plugin);
  ...
  it('keeps one conversation for an inline edit and its follow-up', async () => {
    const harness = createPlugin();
    const service = new KimicodeInlineEditService(harness.plugin);
  ...
  it('refines instructions in a conversation of its own', async () => {
    const harness = createPlugin();
    const service = new KimicodeInstructionRefineService(harness.plugin);
  ...
  it('generates each title on a conversation of its own, and ends it', async () => {
    const harness = createPlugin();
    const service = new KimicodeTitleGenerationService(harness.plugin);
