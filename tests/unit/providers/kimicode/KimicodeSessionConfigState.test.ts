...
import {
  type KimicodeSessionConfigPorts,
  KimicodeSessionConfigState,
} from '@/providers/kimicode/execution/KimicodeSessionConfigState';
import { KIMICODE_DEFAULT_THINKING_LEVEL } from '@/providers/kimicode/models';

/**
 * What `KimicodeChatRuntime` used to hold in five fields, now held here.
 *
 ...
 */
describe('KimicodeSessionConfigState', () => {
  function createState(settings: Record<string, unknown> = {}): {
    state: KimicodeSessionConfigState;
    saved: () => number;
  } {
    let saves = 0;
    const ports: KimicodeSessionConfigPorts = {
    ...
    return { state: new KimicodeSessionConfigState(ports), saved: () => saves };
  }
  ...
  it('wants nothing when the vault is on the default level, or on none', () => {
    expect(createState({ effortLevel: KIMICODE_DEFAULT_THINKING_LEVEL }).state.desiredEffortValue())
      .toBeNull();
