...
/**
 * What one Reasonix turn says, and what the flip must not have changed about it.
 *
 * These assertions lived inside `ReasonixChatRuntime.test.ts` and were driving a
 ...
 */
describe('buildReasonixPrompt', () => {
  it('carries every piece of the vault a turn was composed with', () => {
    const prompt = buildReasonixPromptText({
      browserSelection: {
  ...
  it('rebuilds prior conversation context when a turn carries history', () => {
    ...
    const prompt = buildReasonixPromptText({ text: 'Apply that to DoorTextStyle.' }, history);
  ...
  it('says nothing twice when the turn carries no history', () => {
    ...
    const blocks = buildReasonixPromptBlocks({ text: 'Continue the edit.' });
  ...
  it('takes the flag from the request as well as from the turn options', () => {
    ...
    const fromRequest = buildReasonixPromptBlocks({ text: 'Plan', orchestratorMode: true });
