...
jest.mock('@/providers/reasonix/app/ReasonixWorkspaceServices', () => ({
  maybeGetReasonixWorkspaceServices: jest.fn(() => ({
    commandCatalog: null,
    modelCatalog: {
      isAvailable: () => true,
...
function createContext(discoveredModels: unknown[]) {
  const plugin = {
    ...
    settings: {
      providerConfigs: {
        reasonix: { enabled: true, discoveredModels },
      },
    },
  } as any;
...
function renderTab(discoveredModels: unknown[]) {
  ...
  reasonixSettingsTabRenderer.render(createMockEl(), context);
  onClick.mockRestore();

  return { context, handlers, plugin, setButtonText, setDisabled };
}

describe('ReasonixSettingsTab model refresh', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('lets the user ask the CLI for its current model list', async () => {
    ...
    expect(getReasonixProviderSettings(plugin.settings).discoveredModels).toHaveLength(2);
