...
describe('OpenCode auxiliary requests', () => {
  function createStore(
    environment: Partial<OpencodeAuxiliaryEnvironment> = {},
    observed: OpencodeAuxiliaryRequest[] = [],
  ): { requests: OpencodeExecutionRequests; seen: OpencodeAuxiliaryRequest[] } {
    let next = 0;
    const requests = new OpencodeExecutionRequests(
      () => `ref-${(next += 1)}`,
      ...
      async request => {
        ...
        return {
          ...
          cwd: '/vault',
          environment: { OPENCODE_CONFIG: '/vault/.grimoire/opencode/auxiliary/title-gen/config.json' },
  ...
  it('resolves an auxiliary turn into a launch, a conversation and a session to configure', async () => {
    ...
    await expect(requests.resolveLaunch('ref-2')).resolves.toEqual({
      ...
      cwd: '/vault',
      environment: {
        OPENCODE_CONFIG: '/vault/.grimoire/opencode/auxiliary/title-gen/config.json',
      },
    });
  });
  ...
  it('applies the model per turn, and only when there is one to apply', async () => {
    ...
    const configured = await withModel.requests.resolveAuxiliary(
      withModel.requests.referenceAuxiliary({ ...titleRequest, model: 'opencode:anthropic:sonnet' }),
    );
  ...
  it('changes the restart fingerprint when the instructions change', async () => {
    ...
    // OpenCode reads its system prompt from a file the artifacts write, so a
  ...
  it('refuses a reference it did not mint, and mints each one once', async () => {
    ...
    // for.
    await expect(requests.resolveAuxiliary(requestRef)).rejects.toThrow('Unknown OpenCode auxiliary');
    await expect(requests.resolveAuxiliary('ref-borrowed')).rejects.toThrow('Unknown OpenCode auxiliary');
  });
  ...
  describe('the runner the auxiliary services keep calling', () => {
    ...
    it('passes the caller its cancellation and its stream', async () => {
      ...
      const answer = await harness.runner.query({
        abortController,
        model: 'opencode:anthropic:sonnet',
        systemPrompt: 'Name the conversation.',
      ...
      expect(harness.seen[0].config.model).toBe('opencode:anthropic:sonnet');
    });
