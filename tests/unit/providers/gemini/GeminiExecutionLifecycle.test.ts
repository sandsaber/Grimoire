...
const OWNER = { kind: 'conversation' as const, ownerId: 'gemini-lifecycle' };

describe('Gemini CLI execution lifecycle integration', () => {
  it('persists one stable native run identity through result and terminal ingestion', async () => {
    ...
    const backend = new GeminiExecutionBackend({
      ...
      resultSink: {
        storeResult: async () => ({
          kind: 'committed',
          result: { resultId: 'gemini-result', storage: 'projection' },
        }),
      },
    ...
    expect(registry.getRun(RUN_ID)).toMatchObject({
      ...
      resultRef: { resultId: 'gemini-result', storage: 'projection' },
...
async function waitFor(predicate: () => boolean): Promise<void> {
  ...
  throw new Error('Timed out waiting for Gemini CLI dispatch.');
}
