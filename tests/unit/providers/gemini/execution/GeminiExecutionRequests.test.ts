...
/**
 * The three reference spaces one Gemini turn passes through.
 *
 ...
 */
describe('GeminiExecutionRequests', () => {
  const prompt: readonly AcpContentBlock[] = [{ type: 'text', text: 'Reply with exactly: ok' }];

  function createStore(overrides: Partial<GeminiInvocationEnvironment> = {}): {
    store: GeminiExecutionRequests;
    launches: number;
  } {
    let launches = 0;
    let next = 0;
    const store = new GeminiExecutionRequests(
      () => `ref-${++next}`,
      async () => {
        launches += 1;
        return {
  ...
  it('resolves a turn into the launch and the prompt the backend dispatches', async () => {
    ...
    await expect(store.resolveLaunch(invocation.startupRef)).resolves.toEqual({
      ...
      // `gemini --acp`, and that is what separates this wave from the four
      // before it.
      arguments: ['--acp'],
      cwd: '/vault',
      environment: { GEMINI_API_KEY: 'redacted' },
  ...
  it('holds a prompt no longer than the dispatch that needed it', async () => {
    ...
    await expect(store.resolve(requestRef)).rejects.toThrow('Unknown Gemini request reference.');
  });
  ...
  it('refuses a reference it never minted rather than guessing', async () => {
    ...
    await expect(store.resolve('ref-nonsense'))
      .rejects.toThrow('Unknown Gemini request reference.');
    await expect(store.resolveLaunch('ref-nonsense'))
      .rejects.toThrow('Unknown Gemini startup reference.');
    await expect(store.resolveDynamic('ref-nonsense'))
      .rejects.toThrow('Unknown Gemini dynamic configuration reference.');
  });
  ...
  it('stays bounded when turns are composed and never dispatched', () => {
    ...
    return expect(store.resolve(first)).rejects.toThrow('Unknown Gemini request reference.');
  });
  ...
  it('drops everything it is holding when the composition goes away', async () => {
    ...
    await expect(store.resolve(requestRef)).rejects.toThrow('Unknown Gemini request reference.');
    await expect(store.resolveLaunch(startupRef)).rejects.toThrow('Unknown Gemini startup reference.');
  });
});
