...
/**
 * What one Gemini turn says, and what the flip must not have changed about it.
 *
 * These assertions lived inside `GeminiChatRuntime.test.ts` and were driving a
 * whole runtime to reach a pure function. The runtime is gone; the function is
 * the same one, and this is where it is pinned now.
 */
describe('buildGeminiPrompt', () => {
  ...
  it('rebuilds prior conversation context when a turn carries history', () => {
    ...
    const prompt = buildGeminiPromptText({ text: 'Apply that to DoorTextStyle.' }, history);
  ...
  it('takes the flag from the request as well as from the turn options', () => {
    ...
    const fromRequest = buildGeminiPromptBlocks({ text: 'Plan', orchestratorMode: true });
