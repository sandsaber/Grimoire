...
export const PARITY_SURFACES: ParitySurface[] = [
  ...
  },
  {
    ...
    area: 'provider',
    description:
      'Title generation, instruction refine, and inline edit. One seam for all nine providers: the composition contributes a runner per purpose, the application owns the services, and a provider that contributes nothing is a provider that says it cannot.',
    state: 'wired',
    modules: [
      'src/app/auxiliary/AuxiliaryExecutionOwner.ts',
      'src/app/execution/auxiliaryPurpose.ts',
      'src/core/auxiliary/ProviderAuxiliarySource.ts',
      ...
      'src/core/auxiliary/titleModel.ts',
      'src/core/auxiliary/UnsupportedAuxiliaryServices.ts',
      'src/features/inline-edit/ui/InlineEditModal.ts',
      // Claude's is still a cold SDK query rather than a kernel run, behind the
      // same seam as the rest.
      'src/providers/claude/auxiliary/ClaudeAuxQueryRunner.ts',
      'src/providers/claude/auxiliary/claudeReadOnlyHook.ts',
    ],
  },
  {
