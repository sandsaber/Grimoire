...
describe('MiMoCode execution composition', () => {
  ...
  it('says what a turn that never started needs the person to do', async () => {
    ...
    expect(errors).toEqual([
      'MiMoCode could not open the session this conversation was resumed from. MiMoCode said: '
        + 'Internal error: MiMoCode service failure. Starting a new chat helps only if the session '
        + 'itself is gone.',
    ]);
    execution.dispose();
    await host.dispose();
  ...
  it('says what the agent said when the session was not what stopped it', async () => {
    ...
    expect(chunks
      ...
      'MiMoCode could not open the session this conversation was resumed from. '
        + 'MiMoCode said: Authentication required. Starting a new chat helps only if the '
