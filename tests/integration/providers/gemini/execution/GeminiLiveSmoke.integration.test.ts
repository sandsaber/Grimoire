...
/**
 * The Gemini flip against a real `gemini --acp` process.
 *
 ...
 * never reach it. Run it with `GRIMOIRE_GEMINI_LIVE=1`.
 */
...
live('Gemini live smoke', () => {
  ...
  function createPlugin(vault: string, overrides: Record<string, unknown> = {}): any {
    ...
    if (typeof overrides.permissionMode === 'string') {
      ...
      updateGeminiProviderSettings(settings, { selectedMode: overrides.permissionMode });
      settings.savedProviderPermissionMode = { gemini: overrides.permissionMode };
    }
    return {
      settings,
  ...
  async function createHarness(
    ...
    const vault = reuseVault ?? mkdtempSync(join(tmpdir(), 'grimoire-gemini-live-'));
  ...
  /** The `gemini --acp` processes this run is responsible for. */
  function agents(): string[] {
    return ownedProcesses(command => command.includes('gemini') && command.includes('--acp'))
      .map(row => row.command);
  }
  ...
  it('row 16: runs the turn in the mode the tab is set to', async () => {
    // The fourth review's Critical, against the real CLI. Grimoire's `plan` is
    // not one of this agent's four ids; a turn that forwarded it would be
    // rejected by `session/set_mode` before the prompt was ever sent, and the
    // whole conversation would die on it. What proves the translation landed is
  ...
  it('row 17: fills the model catalog from an empty vault', async () => {
    ...
    const stored = getGeminiProviderSettings(plugin.settings);
    ...
    // The models and the modes in one reply, which is what makes this provider's
    // metadata session the shortest of the five.
  ...
  it('row 19: shows the spend when there is spend to show', async () => {
    geminiPlanUsageStore.reset();
    const { runtime, plugin, shutdown } = await createHarness();

    const chunks = await drain(runtime.query(runtime.prepareTurn({ text: 'Reply with exactly: OK' })));

    const usage = geminiPlanUsageStore.getCachedUsage({
      plugin,
      providerId: 'gemini',
