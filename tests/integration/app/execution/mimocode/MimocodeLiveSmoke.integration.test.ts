...
/**
 * The MiMoCode flip against a real `mimo acp` process.
 *
 ...
 * never reach it. Run it with `GRIMOIRE_MIMOCODE_LIVE=1`.
 */
const live = process.env.GRIMOIRE_MIMOCODE_LIVE === '1' ? describe : describe.skip;

live('MiMoCode live smoke', () => {
  ...
  async function createHarness(
    ...
    const vault = reuseVault ?? mkdtempSync(join(tmpdir(), 'grimoire-mimocode-live-'));
    ...
    const execution = new MimocodeExecution(plugin, host.registry);
  ...
  /** The `mimo acp` processes this run is responsible for. */
  function agents(): string[] {
    // The binary is `mimo`; the directory it ships in is `.mimocode`. Matching
    // the binary matches both, and matching only the directory would miss a
    // `mimo` resolved from anywhere else.
    return ownedProcesses(command => command.includes('mimo') && command.includes('acp'))
      .map(row => row.command);
  }
  ...
  it('row 17: fills the model catalog from an empty vault', async () => {
    ...
    const models = getMimocodeProviderSettings(plugin.settings).discoveredModels;
  ...
  it('row 19: shows the spend when there is spend to show', async () => {
    ...
    const usage = mimocodePlanUsageStore.getCachedUsage({
      plugin,
      providerId: 'mimocode',
