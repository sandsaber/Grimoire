...
/**
 * The Devin chat projection, against a real `devin acp`.
 *
 * Nothing below the composition is a fake — see `chatProjectionLiveHarness`.
 *
 * Off by default. Run it with `GRIMOIRE_DEVIN_LIVE=1`; `devin auth login` first.
 */
const live = process.env.GRIMOIRE_DEVIN_LIVE === '1' ? describe : describe.skip;

live('Devin chat projection live smoke', () => {
  jest.setTimeout(300_000);

  const CONVERSATION_ID = 'conv-devin-projection';
  ...
  function refuseVendorOutage(
    ...
    if (failure && /unavailable|service failure|upstream/i.test(failure)) {
      throw new Error(
        `Devin could not serve this row: ${failure} `
  ...
  function createPlugin(vault: string, overrides: Record<string, unknown> = {}): any {
    ...
    if (process.env.GRIMOIRE_DEVIN_MODEL) {
      settings.model = process.env.GRIMOIRE_DEVIN_MODEL;
    }
    return {
      settings,
  ...
  async function createHarness(
    ...
    const vault = reuseVault ?? mkdtempSync(join(tmpdir(), 'grimoire-devin-projection-'));
    ...
    const execution = new DevinExecution(createPlugin(vault, overrides), host.registry);
  ...
  it('row C: resumes the session after a reload, from what the vault kept', async () => {
    ...
    const shared = mkdtempSync(join(tmpdir(), 'grimoire-devin-projection-reload-'));
    ...
    // Devin's session, like every ACP one, belongs to the project it was opened
  ...
  // Devin reports usage over ACP inside the run it belongs to: `usage_update`
  // before the prompt result (recorded seq 35 before seq 37).
