 ...
 * 2026-08-23 and unchanged. The rows are written and run anyway: this harness
 * found a shipped defect on its first run, on the load path an authenticated
 * account never takes.
 *
 * Nothing below the composition is a fake — see `chatProjectionLiveHarness`.
 *
 * Off by default. Run it with `GRIMOIRE_KIMICODE_LIVE=1`.
 */
const live = process.env.GRIMOIRE_KIMICODE_LIVE === '1' ? describe : describe.skip;

live('Kimi Code chat projection live smoke', () => {
  jest.setTimeout(300_000);

  const CONVERSATION_ID = 'conv-kimicode-projection';
  ...
  it('is about a provider that is on the projection path', () => {
    expect(usesProjectionChat('kimicode')).toBe(true);
  });
  ...
  async function createHarness(
    ...
    const vault = reuseVault ?? mkdtempSync(join(tmpdir(), 'grimoire-kimicode-projection-'));
    ...
    const execution = new KimicodeExecution(createPlugin(vault, overrides), host.registry);
  ...
  it('row C: resumes the session after a reload, from what the vault kept', async () => {
    ...
    const shared = mkdtempSync(join(tmpdir(), 'grimoire-kimicode-projection-reload-'));
