...
/**
 * The Qwen Code chat projection flip, against a real `qwen --experimental-acp`.
 *
 * Qwen is the one provider that asks a **question** rather than only for
 * permission: it sends `ask_user_question` down the ACP permission channel and
 * answers with structured content beside the option id, which is why
 * `InteractionResolution` carries an opaque payload at all. That payload rides
 * the same seam this path's presenter does, so the row that certifies a
 * permission here certifies a question too.
 *
 * **This CLI is not authenticated here**, recorded on 2026-08-23. The rows are
 * written and run anyway: a harness that cannot pass a row still exercises the
 * failure surface.
 *
 * Nothing below the composition is a fake — see `chatProjectionLiveHarness`.
 *
 * Off by default. Run it with `GRIMOIRE_QWEN_LIVE=1`.
 */
const live = process.env.GRIMOIRE_QWEN_LIVE === '1' ? describe : describe.skip;

live('Qwen Code chat projection live smoke', () => {
  jest.setTimeout(300_000);

  const CONVERSATION_ID = 'conv-qwen-projection';
  ...
  it('is about a provider that is on the projection path', () => {
    expect(usesProjectionChat('qwen')).toBe(true);
  });
  ...
  function refuseVendorOutage(chunks: readonly { type: string }[]): void {
    ...
    if (failure && /unavailable|service failure|upstream/i.test(failure.content)) {
      throw new Error(
        `Qwen Code could not serve this row: ${failure.content} `
  ...
  function createPlugin(vault: string, overrides: Record<string, unknown> = {}): any {
    ...
    if (process.env.GRIMOIRE_QWEN_MODEL) {
      settings.model = process.env.GRIMOIRE_QWEN_MODEL;
    }
    return {
      settings,
  ...
  async function createHarness(
    ...
    const vault = reuseVault ?? mkdtempSync(join(tmpdir(), 'grimoire-qwen-projection-'));
    ...
    const execution = new QwenExecution(createPlugin(vault, overrides), host.registry);
  ...
  it('row C: resumes the session after a reload, from what the vault kept', async () => {
    ...
    const shared = mkdtempSync(join(tmpdir(), 'grimoire-qwen-projection-reload-'));
