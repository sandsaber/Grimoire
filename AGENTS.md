...
- A provider process belongs to a kernel session and must be reachable by every release path: the last tab leaving an idle conversation, adapter-owned session disposal, the idle timeout (`ExecutionSession.suspend()`), and unload. Admitted conversation work survives tab detachment and releases its process after persistence. A backend that keeps its process warm between turns implements `suspend()`; see the process-lifetime section of [`docs/provider-execution-adapter-contract.md`](docs/provider-execution-adapter-contract.md).
...
Bump the version in the release commit that immediately precedes the tag, never in a feature branch or a merge to `main`. Obsidian does not read the release list: it reads `manifest.json` from the default branch, takes `version` from it, and downloads the assets of the release tagged with exactly that version. So a `main` carrying a version that has no release offers every installed client an update that cannot be downloaded - which is what happened when the 2.0.0 merge landed on `main` while 1.3.2 was the newest release. Until the next tag is pushed, `main` must keep advertising the newest published version, even when it already holds the next version's code; a `CHANGELOG.md` section drafted ahead of its release is headed `## <version> - Unreleased` rather than dated.
