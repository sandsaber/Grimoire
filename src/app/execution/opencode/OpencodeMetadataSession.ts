...
export class OpencodeMetadataSession {
  ...
  private async withSession(
    ...
    try {
      ...
      const commands = collectAnnouncedCommands(client);
      await client.initialize();
      const opened = await client.newSession({
        cwd: launch.cwd,
        mcpServers: [...launch.mcpServers],
      });
      return await read(client, opened.sessionId, this.createConfigState(), opened, commands);
    } catch {
...
function collectAnnouncedCommands(
  client: ManagedAcpClient,
): Promise<readonly OpencodeMetadataCommand[]> {
  return new Promise(resolve => {
    let settled = false;
    const finish = (commands: readonly OpencodeMetadataCommand[]): void => {
      if (settled) {
        return;
      }
      settled = true;
    ...
    const timer = window.setTimeout(() => finish([]), COMMAND_ANNOUNCEMENT_MS);
  });
}
