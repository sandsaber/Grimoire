...
- `projections/`, `application/` and `rendering/ChatProjection*` are M5's replacement path, and **every chat runs on it**. The composition is constructed at plugin load and each tab builds its end of it; `InputController.sendMessage` has one path, and a tab without a projection refuses to send rather than falling back, because there is nothing to fall back to. It was flipped one provider at a time through `src/app/chat/projectionChatProviders.ts`, certified per provider by `docs/chat-projection-flip-smoke-matrix.md`; that list is deleted now that all nine are on it, because removing an entry would have stopped a provider sending rather than reverting its flip.
