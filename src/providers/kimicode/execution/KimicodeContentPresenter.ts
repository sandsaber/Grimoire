...
import { createKimicodeToolStreamAdapter } from '@/providers/kimicode/normalization/kimicodeToolNormalization';
 ...
 */
export type KimicodeContentPayload = AcpContentPayload;

/** What a session answered with when it was created or loaded. */
export interface KimicodeSessionOpening {
...
export interface KimicodeContentPresenterPorts {
   ...
   *
   * Separate from `onCurrentMode` because the mode reported here is Kimi Code's
   ...
   */
  readonly onSessionOpened?: (opening: KimicodeSessionOpening) => void;
}
 ...
 *
 * `AcpSessionUpdateNormalizer` and Kimi Code's own tool stream adapter are the
 * pieces that know how an ACP tool call, its output, a plan and a usage report
 * are rendered, and `KimicodeChatRuntime` drives exactly these two today. The
 ...
 */
export class KimicodeContentPresenter {
  ...
  constructor(private readonly ports: KimicodeContentPresenterPorts) {}
   ...
   * all. Kimi Code's legacy runtime resets both the normalizer and the tool
  ...
  present(payload: unknown): readonly StreamChunk[] {
    const content = payload as Partial<KimicodeContentPayload> | null;
   ...
   *
   * Kimi Code reports its models, its modes and its config options in the reply
   * to `session/new` and `session/load`, and never again unless something is
   * set. Not from its own recording, which never opened a session — from
   * `KimicodeChatRuntime`, which reads exactly those three off exactly those two
   * replies, and from the three ACP providers already flipped, which all do. A
   * tab whose selectors were fed only from later updates would start empty on a
   * fresh vault and stay empty until the user changed something.
   */
