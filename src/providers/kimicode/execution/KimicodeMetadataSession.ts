...
/** Where an isolated Kimi Code process is launched, and what it is launched for. */
export interface KimicodeMetadataLaunch {
  readonly startupRef: string;
  readonly cwd: string;
  readonly mcpServers: AcpNewSessionRequest['mcpServers'];
}

export interface KimicodeMetadataSessionPorts {
  readonly clientFactory: ManagedAcpClientFactory;
  /** The process to spawn, prepared for this question and no conversation. */
  readonly launch: () => Promise<KimicodeMetadataLaunch>;
...
export interface KimicodeMetadataCommand {
 ...
 * a whole `KimicodeChatRuntime` and driving it as far as a session. That is the
 * only thing the legacy runtime was doing for them: opening a session and
 * reading its reply.
 *
 * Isolated by construction. The launch it asks for points Kimi Code at an
 * in-memory database, so nothing here binds a session to a conversation, writes
 * to the vault's Kimi Code state, or leaves a process behind — the client is
 * closed on every path, including the ones that failed.
 */
export class KimicodeMetadataSession {
  constructor(private readonly ports: KimicodeMetadataSessionPorts) {}
  ...
  async discoverMetadata(options: { readonly rawModelId?: string } = {}): Promise<boolean> {
    return this.withSession(async (client, sessionId, config, opened) => {
      ...
      await config.syncSessionModeState({
        configOptions: opened.configOptions ?? null,
        // The default agent a new session reports is Kimi Code's, not the
   ...
   * on silence.
   */
  async listCommands(): Promise<readonly KimicodeMetadataCommand[]> {
    let announced: readonly KimicodeMetadataCommand[] = [];
  ...
  private async withSession(
    read: (
      ...
      sessionId: string,
      config: KimicodeSessionConfigState,
      opened: Awaited<ReturnType<ManagedAcpClient['newSession']>>,
      commands: Promise<readonly KimicodeMetadataCommand[]>,
    ) => Promise<boolean>,
  ): Promise<boolean> {
  ...
  private createConfigState(): KimicodeSessionConfigState {
    return new KimicodeSessionConfigState({
...
function collectAnnouncedCommands(client: ManagedAcpClient): {
  readonly commands: Promise<readonly KimicodeMetadataCommand[]>;
  readonly arm: () => void;
} {
  let settle: ((commands: readonly KimicodeMetadataCommand[]) => void) | undefined;
  let timer: number | undefined;
  const commands = new Promise<readonly KimicodeMetadataCommand[]>(resolve => {
    settle = resolve;
  });
  const finish = (announced: readonly KimicodeMetadataCommand[]): void => {
    if (!settle) {
      return;
    }
