...
export interface KimicodeAcpFileSystemContext {
...
export class KimicodeAcpFileSystem {
  constructor(private readonly context: KimicodeAcpFileSystemContext) {}
  ...
  async writeTextFile(request: AcpWriteTextFileRequest): Promise<AcpWriteTextFileResponse> {
    ...
    if (!approved) throw new Error('Kimi Code file write was not approved.');
