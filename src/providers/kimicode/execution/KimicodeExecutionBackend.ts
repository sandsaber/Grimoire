...
export interface KimicodeExecutionInvocation {
...
export interface KimicodeExecutionRequestResolver {
  resolve(requestRef: string): Promise<KimicodeExecutionInvocation>;
}

export interface KimicodeExecutionDynamicApplier {
  apply(input: {
...
export interface KimicodePreparedInteraction {
...
export interface KimicodeInteractionBridge {
  prepare(request: AcpRequestPermissionRequest): Promise<KimicodePreparedInteraction>;
}

export interface KimicodeExecutionResultSink {
  storeResult(input: {
...
export interface KimicodeAuxiliaryPort {
  execute(requestRef: string, signal: AbortSignal): Promise<string>;
}

export type KimicodeExecutionScheduler = ResultCommitScheduler;

export interface KimicodeExecutionBackendContext {
  readonly clientFactory: ManagedAcpClientFactory;
  readonly requestResolver: KimicodeExecutionRequestResolver;
  readonly dynamicApplier: KimicodeExecutionDynamicApplier;
  readonly interactionBridge: KimicodeInteractionBridge;
  readonly resultSink: KimicodeExecutionResultSink;
  readonly reconciler: ExecutionRecoveryPort;
  readonly auxiliaryQueries: KimicodeAuxiliaryPort;
  readonly scheduler: KimicodeExecutionScheduler;
...
interface PendingInteraction {
  ...
  readonly run: KimicodeExecutionRun;
  readonly prepared: KimicodePreparedInteraction;
...
export class KimicodeExecutionBackend
implements ExecutionBackend, InteractionPort, ExecutionRecoveryPort {
  readonly descriptor = KIMICODE_EXECUTION_DESCRIPTOR;
  private readonly sessions = new Map<string, KimicodeExecutionSession>();
  ...
  constructor(protected readonly context: KimicodeExecutionBackendContext) {}
  ...
  async createSession(config: ExecutionSessionConfig): Promise<ExecutionSession> {
    ...
    let session: KimicodeExecutionSession;
    session = new KimicodeExecutionSession(
      config,
      this.context,
  ...
  private async requestPermission(
    ...
    run: KimicodeExecutionRun | undefined,
  ...
  private async discardPreparedInteraction(
    prepared: KimicodePreparedInteraction,
  ): Promise<void> {
    await withTimeout(
      prepared.cancel(),
      this.context.scheduler,
  ...
  private cancelRunInteractions(run: KimicodeExecutionRun): void {
...
class KimicodeExecutionSession implements ExecutionSession {
  ...
  private activeRun?: KimicodeExecutionRun;

  constructor(
    private readonly config: ExecutionSessionConfig,
    private readonly context: KimicodeExecutionBackendContext,
    private readonly permissionHandler: (
      request: AcpRequestPermissionRequest,
    ) => Promise<AcpRequestPermissionResponse>,
    private readonly cancelInteractions: (run: KimicodeExecutionRun) => void,
  ...
  get activeExecutionRun(): KimicodeExecutionRun | undefined {
    return this.activeRun;
  }

  createRun(request: ExecutionRequest): ExecutionRun {
    const run = new KimicodeExecutionRun(
      request,
      this,
      this.context,
  ...
  isAttached(run: KimicodeExecutionRun): boolean {
    return this.activeRun === run && Boolean(this.client && this.nativeSessionRef);
  }

  async prepare(run: KimicodeExecutionRun, invocation: KimicodeExecutionInvocation): Promise<void> {
  ...
  dispatch(run: KimicodeExecutionRun, invocation: KimicodeExecutionInvocation): void {
  ...
  async reconcileRun(run: KimicodeExecutionRun): Promise<RunRecoveryEvidence> {
  ...
  private async recover(
    run: KimicodeExecutionRun,
    invocation: KimicodeExecutionInvocation,
    _error: unknown,
    attempt: number,
  ): Promise<void> {
...
class KimicodeExecutionRun implements ExecutionRun {
  ...
  constructor(
    ...
    private readonly session: KimicodeExecutionSession,
    private readonly context: KimicodeExecutionBackendContext,
    private readonly cancelInteractions: (run: KimicodeExecutionRun) => void,
  ...
  openInteraction(interactionId: InteractionId, prepared: KimicodePreparedInteraction): void {
    this.emit({
...
function validateInvocation(invocation: KimicodeExecutionInvocation): void {
...
function validatePreparedInteraction(prepared: KimicodePreparedInteraction): void {
...
function withTimeout<T>(
  operation: Promise<T>,
  scheduler: KimicodeExecutionScheduler,
  timeoutMs: number,
): Promise<T | undefined> {
  return new Promise(resolve => {
    let settled = false;
