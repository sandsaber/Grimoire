...
} from '@/providers/opencode/execution/OpencodeMetadataSession';
    ...
    const ports: ExecutionChatRuntimeHostPorts = {
      ...
      describeFailure: reason => {
        ...
        if (reason === 'provider-failure' || reason === 'pre-dispatch-rejected') {
          ...
          if (refused?.origin) {
            return describeAcpSessionOpenFailure('OpenCode', refused.message, refused.origin);
          }
          if (refused) {
  ...
  auxiliarySource(): ProviderAuxiliarySource {
    return {
      ...
      resolveTitleModel: () => resolveConfiguredTitleModel(
        ...
        (modelId, settings) => opencodeChatUIConfig.ownsModel(modelId, settings),
        decodeOpencodeModelId,
      ),
    };
  }
