...
export interface OpencodeExecutionInvocation {
...
export interface OpencodeExecutionRequestResolver {
  resolve(requestRef: string): Promise<OpencodeExecutionInvocation>;
}

export interface OpencodeExecutionDynamicApplier {
  apply(input: {
...
export interface OpencodePreparedInteraction {
...
export interface OpencodeInteractionBridge {
  prepare(request: AcpRequestPermissionRequest): Promise<OpencodePreparedInteraction>;
}

export interface OpencodeExecutionResultSink {
  storeResult(input: {
...
export interface OpencodeAuxiliaryPort {
  execute(requestRef: string, signal: AbortSignal): Promise<string>;
}

export type OpencodeExecutionScheduler = ResultCommitScheduler;

export interface OpencodeExecutionBackendContext {
  readonly clientFactory: ManagedAcpClientFactory;
  readonly requestResolver: OpencodeExecutionRequestResolver;
  readonly dynamicApplier: OpencodeExecutionDynamicApplier;
  readonly interactionBridge: OpencodeInteractionBridge;
  readonly resultSink: OpencodeExecutionResultSink;
  readonly reconciler: ExecutionRecoveryPort;
  readonly auxiliaryQueries: OpencodeAuxiliaryPort;
  readonly scheduler: OpencodeExecutionScheduler;
...
interface PendingInteraction {
  ...
  readonly run: OpencodeExecutionRun;
  readonly prepared: OpencodePreparedInteraction;
...
implements ExecutionBackend, InteractionPort, ExecutionRecoveryPort {
  readonly descriptor = OPENCODE_EXECUTION_DESCRIPTOR;
  private readonly sessions = new Map<string, OpencodeExecutionSession>();
  ...
  constructor(protected readonly context: OpencodeExecutionBackendContext) {}
  ...
  async createSession(config: ExecutionSessionConfig): Promise<ExecutionSession> {
    ...
    let session: OpencodeExecutionSession;
    session = new OpencodeExecutionSession(
      config,
      this.context,
  ...
  private async requestPermission(
    ...
    run: OpencodeExecutionRun | undefined,
  ...
  private async discardPreparedInteraction(
    prepared: OpencodePreparedInteraction,
  ): Promise<void> {
    await withTimeout(
      prepared.cancel(),
      this.context.scheduler,
  ...
  private cancelRunInteractions(run: OpencodeExecutionRun): void {
...
class OpencodeExecutionSession implements ExecutionSession {
  ...
  private activeRun?: OpencodeExecutionRun;

  constructor(
    private readonly config: ExecutionSessionConfig,
    private readonly context: OpencodeExecutionBackendContext,
    private readonly permissionHandler: (
      request: AcpRequestPermissionRequest,
    ) => Promise<AcpRequestPermissionResponse>,
    private readonly cancelInteractions: (run: OpencodeExecutionRun) => void,
  ...
  get activeExecutionRun(): OpencodeExecutionRun | undefined {
  ...
  isAttached(run: OpencodeExecutionRun): boolean {
    return this.activeRun === run && Boolean(this.client && this.nativeSessionRef);
  }

  async prepare(run: OpencodeExecutionRun, invocation: OpencodeExecutionInvocation): Promise<void> {
  ...
  dispatch(run: OpencodeExecutionRun, invocation: OpencodeExecutionInvocation): void {
  ...
  async reconcileRun(run: OpencodeExecutionRun): Promise<RunRecoveryEvidence> {
...
class OpencodeExecutionRun implements ExecutionRun {
  ...
  constructor(
    ...
    private readonly session: OpencodeExecutionSession,
    private readonly context: OpencodeExecutionBackendContext,
    private readonly cancelInteractions: (run: OpencodeExecutionRun) => void,
  ...
  openInteraction(interactionId: InteractionId, prepared: OpencodePreparedInteraction): void {
    this.emit({
...
function validateInvocation(invocation: OpencodeExecutionInvocation): void {
...
function validatePreparedInteraction(prepared: OpencodePreparedInteraction): void {
...
function withTimeout<T>(
  operation: Promise<T>,
  scheduler: OpencodeExecutionScheduler,
  timeoutMs: number,
): Promise<T | undefined> {
  return new Promise(resolve => {
    let settled = false;
