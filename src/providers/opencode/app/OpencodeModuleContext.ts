 ...
 * not serve.
 */
export type OpencodeBoundConversation = () => BoundConversation | null;

/**
 * The module's context over the running plugin, for the runtime's features.
 *
 * Only the history slots are wired. OpenCode's workspace is still registered
 * the legacy way through `OpencodeWorkspaceServices`, and moving it is a later
...
function notWired(slot: string): Promise<never> {
  return Promise.reject(new Error(
    `OpenCode workspace slot "${slot}" is served by the legacy workspace registration, `
    + 'not by this context.',
  ));
}
