...
- Image attachments have no CLI flag and no content-block channel: `agy --help` exposes nothing for images and the stream-json `user` event carries `content` as a plain string. `AntigravityImageAttachments` therefore writes each attachment into a fresh `os.tmpdir()` directory and appends the absolute paths to the prompt. Measured 2026-09-03 against agy on Windows: the agent opens such a path even when the file lives outside the workspace and no `--add-dir` is passed, so do not couple attachments to the `--add-dir` probe. The temp directory holds user data and is removed when **that tab** dispatches its next turn, and on plugin unload — not at the end of the turn that wrote it. Print mode ends its process without the composition observing it, so files freed at the turn's own end would be pulled out from under an `agy` still reading them; freed per tab, because one composition serves every tab and a second tab's dispatch would otherwise delete a live turn's files. The consequence is that an attachment can outlive its answer: a user who attaches an image, reads the reply and never sends another Antigravity turn leaves the decoded copy in `os.tmpdir()` until Obsidian unloads the plugin, and a force-quit leaves it there — nothing sweeps the `grimoire-antigravity-images-` prefix at startup. An attachment still reaches exactly one turn: print-mode history is text-only, so a follow-up question cannot see the image and the user has to attach it again. An attachment that cannot be written is reported to the user as a `notice` chunk naming it, not only through the debug log, which is off by default.
