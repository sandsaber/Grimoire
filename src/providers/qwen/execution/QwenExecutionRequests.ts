...
import type { QwenAcpDynamicConfig } from './QwenAcpDynamicConfig';
import type { QwenExecutionInvocation } from './QwenExecutionBackend';

/** What one Qwen turn decides, before it becomes an opaque reference. */
export interface QwenExecutionRequest {
  readonly prompt: readonly AcpContentBlock[];
  /** The mode, model and effort this turn runs under, applied to the session. */
  readonly dynamic?: QwenAcpDynamicConfig;
  readonly messageId?: string;
}

/**
 * Everything ambient a launched `qwen --acp` runs under, read at dispatch.
 *
 ...
 */
export interface QwenInvocationEnvironment {
...
/**
 * The store behind Qwen's request, startup and dynamic references.
 *
 ...
 */
export class QwenExecutionRequests {
  private readonly pending = new Map<string, QwenExecutionRequest>();
  private readonly startups = new Map<string, ManagedAcpLaunchInvocation>();
  private readonly dynamics = new Map<string, QwenAcpDynamicConfig>();

  constructor(
    private readonly nextReference: () => string,
    private readonly environment: () => Promise<QwenInvocationEnvironment>,
    private readonly limit: number = DEFAULT_LIMIT,
  ) {}

  /** Holds a turn and returns the reference the kernel will carry. */
  reference(request: QwenExecutionRequest): string {
  ...
  async resolve(requestRef: string): Promise<QwenExecutionInvocation> {
   ...
   *
   * The metadata session is the caller: it opens an isolated Qwen process to
   * ask what models and commands exist, which is a launch with no prompt behind
   * it and therefore no request reference to resolve into one.
   */
  ...
  async resolveLaunch(startupRef: string): Promise<ManagedAcpLaunchInvocation> {
    ...
    if (!launch) {
      throw new Error('Unknown Qwen startup reference.');
    }
    return launch;
  }
   ...
   * then mode, then the `/effort` prompt — stays where it already is.
   */
  async resolveDynamic(dynamicRef: string): Promise<QwenAcpDynamicConfig> {
    const dynamic = this.dynamics.get(dynamicRef);
    if (!dynamic) {
      throw new Error('Unknown Qwen dynamic configuration reference.');
    }
    return dynamic;
  }
  ...
  private take(requestRef: string): QwenExecutionRequest {
    const request = this.pending.get(requestRef);
    if (!request) {
      throw new Error('Unknown Qwen request reference.');
    }
