...
- Chat execution runs through the kernel: `QwenExecution` (`src/providers/qwen/execution/`) owns the backend, the permission bridge, the question bridge and the isolated metadata session, and `registration.ts` points `createRuntime` at it. `QwenChatRuntime` is gone — see `docs/provider-execution-migration-progress.md` for the flip.
