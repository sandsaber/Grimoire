...
export interface GeminiExecutionInvocation {
...
export interface GeminiExecutionRequestResolver {
  resolve(requestRef: string): Promise<GeminiExecutionInvocation>;
}

export interface GeminiExecutionDynamicApplier {
  apply(input: {
...
export interface GeminiPreparedInteraction {
...
export interface GeminiInteractionBridge {
  prepare(request: AcpRequestPermissionRequest): Promise<GeminiPreparedInteraction>;
}

export interface GeminiExecutionResultSink {
  storeResult(input: {
...
export interface GeminiExecutionUsagePort {
  attach(input: {
...
export interface GeminiHistoryReplayPort {
  begin(input: {
    readonly sessionId: string;
    readonly cwd: string;
    readonly signal: AbortSignal;
...
export type GeminiExecutionScheduler = ResultCommitScheduler;

export interface GeminiExecutionBackendContext {
  readonly clientFactory: ManagedAcpClientFactory;
  readonly requestResolver: GeminiExecutionRequestResolver;
  readonly dynamicApplier: GeminiExecutionDynamicApplier;
  readonly interactionBridge: GeminiInteractionBridge;
  readonly resultSink: GeminiExecutionResultSink;
  ...
  readonly scheduler: GeminiExecutionScheduler;
...
interface PendingInteraction {
  ...
  readonly run: GeminiExecutionRun;
  readonly prepared: GeminiPreparedInteraction;
...
export class GeminiExecutionBackend
implements ExecutionBackend, InteractionPort, ExecutionRecoveryPort {
  readonly descriptor = GEMINI_EXECUTION_DESCRIPTOR;
  private readonly sessions = new Map<string, GeminiExecutionSession>();
  ...
  constructor(protected readonly context: GeminiExecutionBackendContext) {}
  ...
  async createSession(config: ExecutionSessionConfig): Promise<ExecutionSession> {
    ...
    let session: GeminiExecutionSession;
    session = new GeminiExecutionSession(
      config,
      this.context,
  ...
  private async requestPermission(
    ...
    run: GeminiExecutionRun | undefined,
  ...
  private async discardPreparedInteraction(
    prepared: GeminiPreparedInteraction,
  ): Promise<void> {
    await withTimeout(
      prepared.cancel(),
      this.context.scheduler,
  ...
  private cancelRunInteractions(run: GeminiExecutionRun): void {
...
class GeminiExecutionSession implements ExecutionSession {
  ...
  private activeRun?: GeminiExecutionRun;
  private controlTurnToken?: object;
  private replacementSession = false;

  constructor(
    private readonly config: ExecutionSessionConfig,
    private readonly context: GeminiExecutionBackendContext,
    private readonly permissionHandler: (
      request: AcpRequestPermissionRequest,
    ) => Promise<AcpRequestPermissionResponse>,
    private readonly cancelInteractions: (run: GeminiExecutionRun) => void,
  ...
  get activeExecutionRun(): GeminiExecutionRun | undefined {
    return this.activeRun;
  }

  createRun(request: ExecutionRequest): ExecutionRun {
    const run = new GeminiExecutionRun(
      request,
      this,
      this.context,
  ...
  isAttached(run: GeminiExecutionRun): boolean {
    return this.activeRun === run && Boolean(this.client && this.nativeSessionRef);
  }

  async prepare(run: GeminiExecutionRun, invocation: GeminiExecutionInvocation): Promise<void> {
    ...
    if (!applied) {
      ...
      if (termination === 'unconfirmed') {
        throw new ExecutionDispatchError(
          'Gemini ACP dynamic configuration ended with unconfirmed process ownership.',
          false,
        );
      }
      throw new ExecutionDispatchError('Gemini ACP dynamic configuration was not confirmed.', true);
    }
  }

  dispatch(run: GeminiExecutionRun, invocation: GeminiExecutionInvocation): void {
  ...
  async reconcileRun(run: GeminiExecutionRun): Promise<RunRecoveryEvidence> {
  ...
  private async recover(
    run: GeminiExecutionRun,
    invocation: GeminiExecutionInvocation,
    _error: unknown,
    attempt: number,
  ): Promise<void> {
  ...
  private async applyDynamic(invocation: GeminiExecutionInvocation): Promise<void> {
    if (!this.client || !this.nativeSessionRef) {
      throw new ExecutionDispatchError('Gemini ACP session is not ready.', true);
    }
    const token = {};
  ...
  private async ensureSessionBinding(
    ...
    if (this.nativeSessionRef && this.loadedSessionRef !== this.nativeSessionRef) {
      ...
      try {
        const controlSignal = this.clientAbort?.signal ?? new AbortController().signal;
        const replayReady = await completesWithin(
          this.context.historyReplay.begin({
            sessionId: target,
            cwd: invocation.cwd,
        ...
        if (!replayReady) {
          const termination = await this.closeClient();
          throw new ExecutionDispatchError(
            'Gemini history replay inventory was not bounded.',
            termination === 'confirmed',
          );
        }
        const loadOutcome = await settleWithin(
          client.loadSession({
            cwd: invocation.cwd,
        ...
        if (loadOutcome.kind === 'timeout') {
          const termination = await this.closeClient();
          throw new ExecutionDispatchError(
            'Gemini ACP session load was not bounded.',
            termination === 'confirmed',
          );
        }
        if (loadOutcome.kind === 'error') throw loadOutcome.error;
        const response = loadOutcome.value;
        ...
        const replaySettled = await completesWithin(
          this.context.historyReplay.settle({
            sessionId: target,
            signal: controlSignal,
          }),
          this.context.scheduler,
        ...
        if (!replaySettled) {
          const termination = await this.closeClient();
          throw new ExecutionDispatchError(
            'Gemini history replay was not bounded.',
            termination === 'confirmed',
          );
        }
        this.requireCurrentClient(client, generation);
        this.loadedSessionRef = target;
        this.attachSessionFeatures(target);
        return;
      } catch (error) {
        this.context.historyReplay.clear(target);
    ...
    if (!this.nativeSessionRef) {
      ...
      this.attachSessionFeatures(response.sessionId);
    }
  }

  private attachSessionFeatures(sessionId: string): void {
    try {
  ...
  private handleNotification(notification: AcpSessionNotification): void {
    ...
    if (this.context.historyReplay.observe(notification)) return;
    try {
  ...
  private async closeCurrentClient(): Promise<'confirmed' | 'unconfirmed'> {
    ...
    if (nativeSessionRef) this.context.historyReplay.clear(nativeSessionRef);
    this.clientGeneration += 1;
...
class GeminiExecutionRun implements ExecutionRun {
  ...
  constructor(
    ...
    private readonly session: GeminiExecutionSession,
    private readonly context: GeminiExecutionBackendContext,
    private readonly cancelInteractions: (run: GeminiExecutionRun) => void,
  ...
  private async finishPrompt(response: AcpPromptResponse, attempt: number): Promise<void> {
    ...
    usageAbort.abort(new Error('Gemini usage projection completed.'));
  ...
  openInteraction(interactionId: InteractionId, prepared: GeminiPreparedInteraction): void {
    this.emit({
...
function validateInvocation(invocation: GeminiExecutionInvocation): void {
  ...
  if (invocation.replacementPrompt && invocation.replacementPrompt.length === 0) {
    throw new ExecutionDispatchError('Gemini replacement prompt is empty.', true);
  }
}

function validatePreparedInteraction(prepared: GeminiPreparedInteraction): void {
...
function withTimeout<T>(
  operation: Promise<T>,
  scheduler: GeminiExecutionScheduler,
  timeoutMs: number,
): Promise<T | undefined> {
  return new Promise(resolve => {
    let settled = false;
