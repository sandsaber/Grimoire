...
- Chat execution runs through the kernel: `GeminiExecution` (`src/providers/gemini/execution/`) owns the backend, the permission bridge and the isolated metadata session, and `registration.ts` points `createRuntime` at it. `GeminiChatRuntime` is gone — see `docs/provider-execution-migration-progress.md` for the flip.
