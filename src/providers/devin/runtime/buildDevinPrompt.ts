...
/**
 * What one Devin turn says, and the eight pieces of the vault in it.
 *
 * Moved out of `DevinChatRuntime` before the flip deleted that file, because a
...
interface DevinPromptOptions {
  orchestratorMode?: boolean;
}

export function buildDevinPromptText(
 ...
 */
export function buildDevinPromptBlocks(
  request: ChatTurnRequest,
  conversationHistory: ChatMessage[] = [],
  options: DevinPromptOptions = {},
): AcpContentBlock[] {
  const prompt = buildDevinPromptText(request, conversationHistory);
