/**
 * How an opened ACP approval reaches the surface, and comes back as an id.
 *
 * Built for OpenCode and shared from wave 5: nothing in it is one provider's.
 * The vocabulary a prompt is worded in reaches it through the bridge, and what
 * it does with the answer — a picked option, a plain decision, a dismissal, a
 * surface that could not ask — is the same for every agent that speaks ACP.
 */
 ...
 */
export type AcpApprovalCallbacks = () => Readonly<ExecutionInteractionCallbacks>;
...
/**
 * An opened ACP permission request, rendered by the surface that already knows
 * how.
 *
 ...
 */
export class AcpApprovalPresenter {
  /** What is on screen right now, so it can be taken down again. */
  private readonly open = new Map<string, AbortController>();

  constructor(
    private readonly bridge: Pick<AcpPermissionBridge, 'presentation'>,
    private readonly callbacks: AcpApprovalCallbacks,
  ) {}
 ...
 */
function toDecisionOption(option: AcpApprovalOption): ApprovalDecisionOption {
  return {
    label: option.label,
