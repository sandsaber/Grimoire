 ...
 */
export type MimocodeAuxiliaryPurpose = 'inline' | 'instructions' | 'title-gen';

/** What one auxiliary turn asks for, before it becomes an opaque reference. */
export interface MimocodeAuxiliaryRequest {
  readonly purpose: MimocodeAuxiliaryPurpose;
  /**
   ...
   *
   * Not a per-turn field despite arriving on every call: MiMoCode reads its
...
/**
 * Everything ambient a launched auxiliary `mimocode acp` runs under.
 *
 ...
 */
export interface MimocodeAuxiliaryEnvironment {
...
export class MimocodeExecutionRequests {
  ...
  private readonly auxiliary = new Map<string, MimocodeAuxiliaryRequest>();
   ...
   */
  referenceAuxiliary(request: MimocodeAuxiliaryRequest): string {
  ...
  async resolveAuxiliary(requestRef: string): Promise<ManagedAcpAuxiliaryInvocation> {
    ...
    if (!request) {
      throw new Error('Unknown MiMoCode auxiliary request reference.');
    }
    this.auxiliary.delete(requestRef);
    if (!this.auxiliaryEnvironment) {
      throw new Error('MiMoCode auxiliary execution has no environment.');
    }
...
export function auxiliaryRetentionKey(
  purpose: MimocodeAuxiliaryPurpose,
  conversationId: string,
): string {
  return `mimocode-auxiliary:${purpose}:${conversationId}`;
}
