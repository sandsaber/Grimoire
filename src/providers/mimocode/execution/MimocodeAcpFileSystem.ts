...
export interface MimocodeAcpFileSystemContext {
...
export class MimocodeAcpFileSystem {
  constructor(private readonly context: MimocodeAcpFileSystemContext) {}
  ...
  async writeTextFile(request: AcpWriteTextFileRequest): Promise<AcpWriteTextFileResponse> {
    ...
    if (!approved) throw new Error('MiMoCode file write was not approved.');
