...
import type { MimocodeExecutionResultSink } from './MimocodeExecutionBackend';

/**
 * The reference for a MiMoCode result, and deliberately not the result.
 *
 * The answer is already durable twice — in the conversation Grimoire persists
 * and in MiMoCode's own session database, which this provider already reads
 ...
 * what MiMoCode itself can be asked about: a run id means nothing to a database
 * this sink does not write.
 */
export class MimocodeProjectionResultSink implements MimocodeExecutionResultSink {
