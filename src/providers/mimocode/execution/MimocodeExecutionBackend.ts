...
export const MIMOCODE_EXECUTION_DESCRIPTOR = Object.freeze({
  backendId: executionBackendId('provider-mimocode'),
  association: { kind: 'provider' as const, providerId: 'mimocode' },
});

export interface MimocodeExecutionInvocation {
  ...
  readonly availableRawModelIds?: readonly string[];
}

export interface MimocodeExecutionRequestResolver {
  resolve(requestRef: string): Promise<MimocodeExecutionInvocation>;
}

export interface MimocodeExecutionDynamicApplier {
  apply(input: {
...
export interface MimocodePreparedInteraction {
...
export interface MimocodeInteractionBridge {
  prepare(request: AcpRequestPermissionRequest): Promise<MimocodePreparedInteraction>;
}

export interface MimocodeExecutionResultSink {
  storeResult(input: {
...
export type MimocodeEmptyResultOutcome =
  | { readonly kind: 'no-provider-error' }
  | { readonly kind: 'provider-failure' }
  | { readonly kind: 'fallback-applied'; readonly fallbackRawModelId: string };

export interface MimocodeEmptyResultPolicy {
  resolve(input: {
...
export interface MimocodeAuxiliaryPort {
  execute(requestRef: string, signal: AbortSignal): Promise<string>;
}

export type MimocodeExecutionScheduler = ResultCommitScheduler;

export interface MimocodeExecutionBackendContext {
  readonly clientFactory: ManagedAcpClientFactory;
  readonly requestResolver: MimocodeExecutionRequestResolver;
  readonly dynamicApplier: MimocodeExecutionDynamicApplier;
  readonly interactionBridge: MimocodeInteractionBridge;
  readonly resultSink: MimocodeExecutionResultSink;
  readonly emptyResultPolicy: MimocodeEmptyResultPolicy;
  readonly reconciler: ExecutionRecoveryPort;
  readonly auxiliaryQueries: MimocodeAuxiliaryPort;
  readonly scheduler: MimocodeExecutionScheduler;
...
interface PendingInteraction {
  ...
  readonly run: MimocodeExecutionRun;
  readonly prepared: MimocodePreparedInteraction;
...
export class MimocodeExecutionBackend
implements ExecutionBackend, InteractionPort, ExecutionRecoveryPort {
  readonly descriptor = MIMOCODE_EXECUTION_DESCRIPTOR;
  private readonly sessions = new Map<string, MimocodeExecutionSession>();
  ...
  constructor(protected readonly context: MimocodeExecutionBackendContext) {}
  ...
  async createSession(config: ExecutionSessionConfig): Promise<ExecutionSession> {
    ...
    let session: MimocodeExecutionSession;
    session = new MimocodeExecutionSession(
      config,
      this.context,
  ...
  private async requestPermission(
    ...
    run: MimocodeExecutionRun | undefined,
  ...
  private async discardPreparedInteraction(
    prepared: MimocodePreparedInteraction,
  ): Promise<void> {
    await withTimeout(
      prepared.cancel(),
      this.context.scheduler,
  ...
  private cancelRunInteractions(run: MimocodeExecutionRun): void {
...
class MimocodeExecutionSession implements ExecutionSession {
  ...
  private activeRun?: MimocodeExecutionRun;

  constructor(
    private readonly config: ExecutionSessionConfig,
    private readonly context: MimocodeExecutionBackendContext,
    private readonly permissionHandler: (
      request: AcpRequestPermissionRequest,
    ) => Promise<AcpRequestPermissionResponse>,
    private readonly cancelInteractions: (run: MimocodeExecutionRun) => void,
  ...
  get activeExecutionRun(): MimocodeExecutionRun | undefined {
    return this.activeRun;
  }

  createRun(request: ExecutionRequest): ExecutionRun {
    const run = new MimocodeExecutionRun(
      request,
      this,
      this.context,
  ...
  isAttached(run: MimocodeExecutionRun): boolean {
    return this.activeRun === run && Boolean(this.client && this.nativeSessionRef);
  }

  async prepare(run: MimocodeExecutionRun, invocation: MimocodeExecutionInvocation): Promise<void> {
  ...
  dispatch(run: MimocodeExecutionRun, invocation: MimocodeExecutionInvocation): void {
  ...
  async reconcileRun(run: MimocodeExecutionRun): Promise<RunRecoveryEvidence> {
  ...
  private async recover(
    run: MimocodeExecutionRun,
    invocation: MimocodeExecutionInvocation,
    _error: unknown,
    attempt: number,
  ): Promise<void> {
...
class MimocodeExecutionRun implements ExecutionRun {
  ...
  constructor(
    ...
    private readonly session: MimocodeExecutionSession,
    private readonly context: MimocodeExecutionBackendContext,
    private readonly cancelInteractions: (run: MimocodeExecutionRun) => void,
  ...
  beginDispatch(sessionRef: string, nativeRunRef?: string): number {
    ...
    this.promptStartedAt = (this.context.now ?? Date.now)();
    this.attempt += 1;
  ...
  async completeFromPrompt(response: AcpPromptResponse, attempt: number): Promise<void> {
    ...
    if (!output) {
      this.completionTask = this.finishFromEmptyResult(attempt);
  ...
  openInteraction(interactionId: InteractionId, prepared: MimocodePreparedInteraction): void {
    this.emit({
  ...
  private async finishFromEmptyResult(attempt: number): Promise<void> {
    const abort = new AbortController();
    const resolution = this.context.emptyResultPolicy.resolve({
      requestRef: this.request.requestRef,
      nativeSessionRef: this.sessionRef!,
      ...(this.nativeRunRef ? { nativeRunRef: this.nativeRunRef } : {}),
      ...(this.invocation.databasePath ? { databasePath: this.invocation.databasePath } : {}),
      availableRawModelIds: this.invocation.availableRawModelIds ?? [],
      startedAt: this.promptStartedAt,
      signal: abort.signal,
    });
    ...
    if (!outcome) abort.abort(new Error('MiMoCode empty-result inspection timed out.'));
    if (this.terminal || attempt !== this.attempt || this.terminationIntent) return;
    if (!outcome || outcome.kind === 'no-provider-error') {
...
function validateInvocation(invocation: MimocodeExecutionInvocation): void {
...
function validatePreparedInteraction(prepared: MimocodePreparedInteraction): void {
...
function withTimeout<T>(
  operation: Promise<T>,
  scheduler: MimocodeExecutionScheduler,
  timeoutMs: number,
): Promise<T | undefined> {
  return new Promise(resolve => {
    let settled = false;
