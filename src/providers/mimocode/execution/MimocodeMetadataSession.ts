...
/** Where an isolated MiMoCode process is launched, and what it is launched for. */
export interface MimocodeMetadataLaunch {
  readonly startupRef: string;
  readonly cwd: string;
  readonly mcpServers: AcpNewSessionRequest['mcpServers'];
}

export interface MimocodeMetadataSessionPorts {
  readonly clientFactory: ManagedAcpClientFactory;
  /** The process to spawn, prepared for this question and no conversation. */
  readonly launch: () => Promise<MimocodeMetadataLaunch>;
...
export interface MimocodeMetadataCommand {
 ...
 * a whole `MimocodeChatRuntime` and driving it as far as a session. That is the
 * only thing the legacy runtime was doing for them: opening a session and
 * reading its reply.
 *
 * Isolated by construction. The launch it asks for points MiMoCode at an
 * in-memory database, so nothing here binds a session to a conversation, writes
 * to the vault's MiMoCode state, or leaves a process behind — the client is
 * closed on every path, including the ones that failed.
 */
export class MimocodeMetadataSession {
  constructor(private readonly ports: MimocodeMetadataSessionPorts) {}
  ...
  async discoverMetadata(options: { readonly rawModelId?: string } = {}): Promise<boolean> {
    return this.withSession(async (client, sessionId, config, opened) => {
      ...
      await config.syncSessionModeState({
        configOptions: opened.configOptions ?? null,
        // The default agent a new session reports is MiMoCode's, not the
   ...
   * on silence.
   */
  async listCommands(): Promise<readonly MimocodeMetadataCommand[]> {
    let announced: readonly MimocodeMetadataCommand[] = [];
  ...
  private async withSession(
    read: (
      ...
      sessionId: string,
      config: MimocodeSessionConfigState,
      opened: Awaited<ReturnType<ManagedAcpClient['newSession']>>,
      commands: Promise<readonly MimocodeMetadataCommand[]>,
    ) => Promise<boolean>,
  ): Promise<boolean> {
  ...
  private createConfigState(): MimocodeSessionConfigState {
    return new MimocodeSessionConfigState({
...
function collectAnnouncedCommands(client: ManagedAcpClient): {
  readonly commands: Promise<readonly MimocodeMetadataCommand[]>;
  readonly arm: () => void;
} {
  let settle: ((commands: readonly MimocodeMetadataCommand[]) => void) | undefined;
  let timer: number | undefined;
  const commands = new Promise<readonly MimocodeMetadataCommand[]>(resolve => {
    settle = resolve;
  });
  const finish = (announced: readonly MimocodeMetadataCommand[]): void => {
    if (!settle) {
      return;
    }
