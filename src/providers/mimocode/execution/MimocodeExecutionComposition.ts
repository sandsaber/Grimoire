...
} from '@/providers/mimocode/execution/MimocodeMetadataSession';
...
export class MimocodeExecution {
  ...
  private readonly workspaceHolder = new ProviderWorkspaceHolder(
    mimocodeProviderModule.workspace,
    () => createMimocodeModuleContext(this.plugin, () => null,
      {
    ...
    const ports: ExecutionChatRuntimeHostPorts = {
      ...
      syncConversation: next => {
        if (next?.id !== conversation?.id) {
          ...
          sessionDropped = getMimocodeState(next?.providerState).sessionDropped === true;
        }
        conversation = next;
      },
      /**
      ...
      describeFailure: reason => {
        ...
        if (reason === 'provider-failure' || reason === 'pre-dispatch-rejected') {
          ...
          if (refused?.origin) {
            return describeAcpSessionOpenFailure('MiMoCode', refused.message, refused.origin);
          }
          if (refused) {
