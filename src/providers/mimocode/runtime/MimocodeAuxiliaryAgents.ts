import type { MimocodeManagedAgentConfig } from './MimocodeLaunchArtifacts';

/**
 * The Grimoire-managed agents an auxiliary MiMoCode turn runs as.
 *
 ...
 * for it to ask on. MiMoCode has no per-request permission mode, so what it can
 ...
 */
export type MimocodeAuxAgentProfile = 'passive' | 'readonly';

export const MIMOCODE_AUX_AGENT_IDS: Readonly<Record<MimocodeAuxAgentProfile, string>> = {
...
export function buildMimocodeAuxAgentConfig(
  profile: MimocodeAuxAgentProfile,
): MimocodeManagedAgentConfig {
