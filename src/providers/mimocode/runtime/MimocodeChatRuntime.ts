...
} from '../execution/MimocodePermissionPresentation';
import { MimocodeSessionConfigState } from '../execution/MimocodeSessionConfigState';
import {
...
const MIMOCODE_SESSION_CWD_LIMIT = 256;
...
export class MimocodeChatRuntime implements ChatRuntime {
   ...
   */
  private readonly sessionConfig = new MimocodeSessionConfigState({
  ...
  private setSessionCwd(sessionId: string, cwd: string): void {
    ...
    trimOldestMapEntries(this.sessionCwds, MIMOCODE_SESSION_CWD_LIMIT);
  }
  ...
  private async persistFallbackModelSelection(rawModelId: string): Promise<void> {
    ...
    this.sessionConfig.forgetSessionModel();
