...
import type { ClaudeExecutionResultSink } from './ClaudeExecutionBackend';

/**
 * The reference for a Claude result, and deliberately not the result.
 *
 * The answer is already durable twice — in the conversation Grimoire persists
 * and in the SDK's own transcript — and D2 forbids a second copy of a provider
 ...
 * this sink is told only the run.
 *
 * A run can commit more than one result: its own answer, and one for every
 * native agent that finishes inside it. So the identity is per source — one
 * identity for two different answers is the failure this exists to prevent.
 */
export class ClaudeProjectionResultSink implements ClaudeExecutionResultSink {
