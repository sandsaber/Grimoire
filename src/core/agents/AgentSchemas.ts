...
function decodeOwner(value: unknown): ExecutionOwner {
  ...
  // **`work-graph` is not an owner here, and the ban is why.** The harvest this
  // came from could root an agent in a graph; M5 excludes graphs, their
  // scheduler and their synthesis runs outright, so a record naming one is a
  // record this build must refuse rather than quietly accept and half-honour.
  const kind = oneOf(record.kind, ['conversation'], 'agent root owner kind');
