...
The feature layer also contains a second execution path outside provider runtimes. `BangBashService` — since deleted, which is what this paragraph argued for — started a local side-effecting process, and `Tab` owned that service directly. It has no durable run identity, recovery state, or application-scoped cancellation owner. A provider-only lifecycle would therefore leave an existing executor behind. The shared execution identity must be a generic backend identity, with provider association as one backend kind and local shell execution as an internal kind.
...
After the topology proofs, production flips proceed per provider: each flip replaces that provider's runtime registration with the presentation adapter over its new backend, passes that provider's trace parity and a manual smoke check, and deletes the provider's legacy `*ChatRuntime` implementation in the same checkpoint. During this window different providers may run on different execution authorities; this is acceptable because providers share no execution state. Inside one provider there is never more than one **chat execution** authority at any commit, and there are no runtime flags — a flip that cannot pass is reverted as a commit. The chat qualifier matters: a flip moves only chat execution, while the provider's auxiliary execution services (title, refine, inline edit) and workspace services stay on the legacy path until their own milestones. That bounded mixed state is planned; contention between the new chat backend and a legacy auxiliary path for the same provider session or process is a stop condition.

The adapter itself is specified before it is built: a method-by-method mapping of the ~27 `ChatRuntime` members to session/run operations, capability ports, or explicit absences, written and checked against the characterized `InputController` expectations during the baseline milestone. The two known hard mappings — synchronous `cancel(): void` against asynchronous run terminals, and generator end against explicit terminal facts — are resolved on paper there; the adapter closes the turn generator only on a terminal fact, which retires "iterator end means success" by construction.

The provider control plane (catalog, settings codecs, lazy workspaces) then replaces the split registries consumer by consumer, with the checked-in contribution inventory (16 registration fields, 11 workspace members, and 3 registration/app-level contributions including the workspace capability record, the separate default-config source, and the initialize/dispose lifecycle) and the parity manifest tracking every contribution each registration carries — the class of dependency whose silent loss broke the first attempt. Durable agent instances and attempts, projections, revisioned conversation persistence in production, and background work that survives tab close are a subsequent presentation-evolution series, migrated surface by surface. The dependency work graph and synthesis runs described in this document remain the target model but are post-migration scope: they are built on the delivered agent domain when a real dependent workflow exists, and they do not gate the removal of the old contract. A visual UI redesign is likewise deliberately post-migration: because projections carry no presentation vocabulary, a new UI is a renderer swap over unchanged contracts rather than another architecture event. The `ChatRuntime` interface, the presentation adapter, and the remaining lifecycle vocabulary of the old contract are deleted when their last UI consumer is gone; deletion gates apply at that point, not to an end-of-branch sweep.
