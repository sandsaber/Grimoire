...
| `runtimeCommandLoader` | 2 | 3 | reshaped | **Done.** `listForSession(sessionId)` presumed a session exists, while the row's context carries `allowSessionCreation` — discovery may *start* a short-lived session, and the tab manager decides when that is allowed. The slot carries the row's two members beside it now, both optional, because five of the nine have no discovery beyond the open session. What made the reshape possible is that the context stopped naming a plugin: it was there only to reach `plugin.getXExecution().metadata.listCommands()`, and a provider's workspace services are built with a plugin, so that call is a closure they capture. Every remaining input is the host's. |
...
| `runtimeCommandLoader` | ~~**The seam.**~~ **Resolved.** Its context carried a `ChatRuntime`, which the seam deletion replaced with the adapter, and its fallback reached `plugin.getXExecution().metadata.listCommands()` — which is now a closure the provider's own workspace services capture, so the context names no plugin. |
