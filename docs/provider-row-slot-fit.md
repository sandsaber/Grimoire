...
**Two rows have moved.** `modelCatalog` was the one whose slot fitted, and it was blocked twice over
...
`getModelCatalog` and `getUsageProvider` are deleted from the registry. Every other remaining row needs its slot reshaped first, and
