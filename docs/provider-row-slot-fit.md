...
**Seven rows have moved**, counting the app-level workspace capability row. `modelCatalog` was the one whose slot fitted, and it was blocked twice over
...
2. **Reshape eight slots.** Design, from the implementations, with the notes above as input.
   `chatUIConfig`, `settingsReconciler` and `commandCatalog` were three of the eleven, and all
   three were reshaped and carried through inside the same milestone.

**Two of the eight wait on their consumers rather than on design.** `taskResultInterpreter` and
