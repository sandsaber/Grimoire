...
| M6 — final hardening | **Code complete; the manual test-vault matrix is the owner's.** Crash injection is done at the execution kernel's durable boundaries and on the agent side, where it found that the agent control store was never recovered at load and that a run left `dispatching` or `running` by a quit kept claiming to run — both fixed, so M5's "agents survive restart with honest classification" now holds in the composition. All seven named behaviours have a test, and the two that live in the shared conformance suite are covered by a condition rather than a claim. The sanitized recordings run end to end through the composition: four of the nine carry a turn, all four reach one `succeeded` terminal and draw content, and the count is asserted. Lazy provider initialization is measured. The privacy review widened the redaction gate to the composition root, which it had never read, and then a review found three of its own warnings arriving as `[redacted-string]`. Three independent reviews returned 11, 12 and their fixes; every finding held or is recorded. Open, each with an owner in its entry: the reconciled-result path has no producer in either domain, three sequential scans of the agent store run on every load, a provider answering `session/new` with an id the control schema refuses stalls its turn instead of failing it, and one recovery protocol is written twice | `fe96ca5`, `9ec2072`, `62359ee`, `2c10e5b`, `ca695c5`, `dbff906`, `64cf348`, `4aa8856`, this commit |
