...
| M6 — final hardening | **Complete, and `main` is merged in.** The manual test-vault matrix is the owner's and the only clause nothing here can answer. Crash injection is done at the execution kernel's durable boundaries and on the agent side, where it found that the agent control store was never recovered at load and that a run left `dispatching` or `running` by a quit kept claiming to run — both fixed, so M5's "agents survive restart with honest classification" now holds in the composition. All seven named behaviours have a test, and the two that live in the shared conformance suite are covered by a condition rather than a claim. The sanitized recordings run end to end through the composition: four of the nine carry a turn, all four reach one `succeeded` terminal and draw content, and the count is asserted. Lazy provider initialization is measured. The privacy review widened the redaction gate to the composition root, which it had never read, and then a review found three of its own warnings arriving as `[redacted-string]`. Four independent reviews returned 11, 12, 13 and 15 findings; all but one held, and every one is fixed or recorded — the fourth caught the third's own fix latching on every read, which would have frozen agent writes for a session after a card refresh stepped over one record. Open, each with an owner in its entry: the reconciled-result path has no producer in either domain, three sequential scans of the agent store run on every load, a provider answering `session/new` with an id the control schema refuses stalls its turn instead of failing it, one recovery protocol is written twice, a replayed delete batch can remove a record this build cannot read, and the latch is a second copy of the registry's where one `ControlStoreReadability` would serve both | `fe96ca5`, `9ec2072`, `62359ee`, `2c10e5b`, `ca695c5`, `dbff906`, `64cf348`, `4aa8856`, `7870cd0`, `b632a74`, and the `main` sync at `cbe8ee9` |
...
lazy provider initialization measured; the privacy review; **four** independent architecture reviews,
the last of which caught the one before it. Seven defects were found and fixed, five of them ones a
user would have met:
...
- D5 was enforced on the execution store and unreachable on the agent one, then enforced as four
  patches with two write paths uncovered, and is now one latch that recovery declares and every
  write consults;
- and that latch, in its first form, was set by *every read* — so a card refresh that stepped over
  one unreadable record froze every agent write for the session, which is the "agent running
  forever" defect coming back through the fix for a different one. A read is not a verdict.
...
recovery protocol is written twice, the D5 latch stops at the first unreadable read rather than
...
| ~~`the two registries` (30)~~ **superseded 2026-08-28 — 17, and the blocker was not durable agents.** | The two rows *were* shaped for consumers durable agents replaces, and that turned out not to matter: both are pure functions of their argument with synchronous consumers, so they are `ProviderDeclarations.asyncTaskResults` and `ProviderDeclarations.subagentLifecycle`, and `ProviderRegistry` is deleted along with the nine `registration.ts` files. What is left is `ProviderWorkspaceRegistry`, whose three remaining accessors each have their own blocker — the command loader's session-opening policy needs a plugin and a runtime, three chat widgets hold the MCP manager by identity, two synchronous settings render paths face an asynchronous lookup — plus the storage role. |
...
| `core importing the plugin type` (2) → **1** | `ProviderWorkspaceRegistry` named it for one method, to assemble a plugin-shaped context its only caller was already holding a plugin to supply; the plugin assembles it now. What is left is `types.ts`, where `ProviderWorkspaceInitContext` is written. |
| `turn metadata and session updates` (6) → **2** | `buildSessionUpdates` still waits on the coordinator's persistence barrier, which carries the execution session id and not the provider's **native** session ref; touching it is conversation-persistence work. `syncConversationState` is settled — it is how a tab's adapter learns which conversation it is on, which is how it drops a session rather than filing one chat's runs under another's name (D4) — so it left the search rather than moving. |
| ~~`StreamChunk` lifecycle vocabulary (5)~~ **superseded 2026-08-28 — the framing is deleted, and it was not smoke-matrix work.** | The blocker was misread. Removing the framing was blocked not by the tests written from real transcripts but by `wireVocabularyCoverage`, which counted *any* chunk a presenter returned as proof an update was modelled — so deleting the framing would have filed `agent_message_chunk` under "nothing draws the surface from this" for an update the kernel consumes. Teaching that gate the two routes it was missing cleared it without a live run, and three of the five variants are now gone. `error` and `done` remain, and their blocker is different again: the adapter's generator, which nothing in `src/` calls and nine live-smoke harnesses do. |
...
agent a provider started and record what it did; it cannot start one. That is the whole of what
stands between orchestrator workers and durable attempts — a worker without a tab needs a turn
dispatched without a tab, and every path to `submitTurn` runs through a tab's encoder and its
adapter. The port's first implementation unlocks `worker tab ownership: 3`, and nothing else does.

**The plan's other precondition for that step is already met, and it is worth saying so because the
plan words the two as one sentence.** "Tab close stops cancelling background work only in the same
checkpoint that ships the durable-ownership UI" — tab close already does not cancel background work:
`destroyTab` cancels the streaming turn and detaches the view, with a comment saying a tab that
stopped drawing is not a tab that stopped the turn; nothing sets `orphaned` any more, which is why
half of that row's own pattern finds nothing; and the background work card reads the vault by
conversation, so an agent outlives the tab that started it and is still findable from another.

**The M5 status row is refreshed** — it still said two of eleven searches were zero and listed
registry deletion and the provider rows as open.

- Gates: unit 8749 passed, 8749 total (553 suites); `tsc --noEmit` clean; `npm run lint` clean.
