...
| 2026-08-31 | 1.18.19 | live: 1, 2, 5, 6, 7, 8, 9, 12/13, 15, 17, 18, 19 | — | twelve of twelve, and run as the check on a change one layer under this provider: a failed ACP filesystem request now carries the handler's own message, and a read of a missing file answers `-32002 Resource not found` where it used to answer an internal error with whatever `node:fs` said. **Row 15 is the one to read** — the permission for a file that does not exist was still raised, and the refusal still stopped the write, which is the row that fails on Gemini for the vendor reason its own matrix records. The 2026-08-30 run of this file (also twelve of twelve) went into the journal and never reached this table, which is what this table is for |
