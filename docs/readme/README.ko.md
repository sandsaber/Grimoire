...
<p align="center">
  <img src="../../assets/readme/chat-workspace.png" alt="Obsidian의 Ocean Atlas 옆에서 해양 층, 해류, 해양 생물을 연결하는 Codex Astra High 채팅" width="100%">
</p>
...
- [Qwen Code 문서](https://qwenlm.github.io/qwen-code-docs/en/)
- [Qwen Code 인증](https://qwenlm.github.io/qwen-code-docs/en/users/configuration/auth/)
...
설치된 CLI에서 선택한 모델의 추론 강도 수준을 가져옵니다. 선택기에는 해당 모델이 지원하는 수준과 CLI 기본값만 표시되며, 강도를 조절할 수 없는 모델에는 선택기가 없습니다. 명시적 선택은 네이티브 세션 수정 API를 통해 현재 실행에 적용되며 CLI 전역 설정은 바꾸지 않습니다. CLI 기본값은 재개한 세션에 저장된 강도를 포함한 네이티브 동작을 유지합니다.
...
**Safe**는 편집, 명령 및 읽기 전용이 아닌 도구를 일시 중지하고 Grimoire에서 일회성 승인을 요청합니다. 거부, 취소 또는 승인 연결이 끊어지면 실행하지 않습니다. 현재 Safe에는 검증된 Command Code 1.53.0 / 1.66.0 npm 설치가 필요하며, 별도 실행 루프가 이 승인 연결을 사용할 수 없는 네이티브 하위 에이전트는 비활성화됩니다. **Auto-approve**는 Grimoire의 확인 없이 실행되지만, 네이티브 거부 및 확인 규칙은 두 모드 모두에 적용됩니다. 이 연동은 대화형 질문, 계획 제어, 슬래시 명령, 관리형 MCP/스킬/에이전트, 보조 작업, 분기, 되감기 또는 네이티브 기록 가져오기를 제공하지 않습니다.
...
- [OpenCode 제공자 문서](https://opencode.ai/docs/providers)
...
- [Grok Build 문서](https://docs.x.ai/build/overview)
...
탭 작업 메뉴를 열거나 탭을 마우스 오른쪽 버튼으로 클릭하여 이름을 바꾸고, 복제하거나 닫을 수 있습니다. 가운데 버튼으로 탭을 닫고 실행 취소로 초안과 위치를 복원합니다. 기록 팝오버에서 이전 채팅을 검색하고 새 탭에서 대화를 열 수 있습니다. 기록은 직접 지정한 제목과 자동 생성된 제목을 구분합니다. 대화 도구 모음에는 이동 버튼과 목차가 있으며, 완료된 답변에는 완료 시각이 표시됩니다.

<p align="center">
  <img src="../../assets/readme/conversation-history.png" alt="세 개의 Ocean Atlas 대화와 모델 및 제목 출처를 보여 주는 기록 검색" width="100%">
</p>
...
| `Escape` | 진행 중인 응답을 중지하거나 기록 팝오버를 닫습니다. |
...
설정은 네 탭으로 나뉩니다. **일반**에서는 언어, 채팅 위치, 탭 및 표시를, **제공업체**에서는 CLI 활성화와 모델을, **고급**에서는 컨텍스트, 대화, 도구 및 진단을, **정보**에서는 버전과 변경 사항을 관리합니다. Obsidian의 기본 설정 검색도 지원합니다.

제공자 개요에는 감지된 CLI와 활성화 스위치가 표시됩니다. 선택한 제공자의 설정은 아래에 나타납니다.

<p align="center">
  <img src="../../assets/readme/settings-providers.png" alt="Grimoire의 열두 CLI 연동과 Codex 모델 설정 개요" width="100%">
</p>
...
<p align="center">
  <img src="../../assets/readme/settings-general.png" alt="언어, 패널 위치, 채팅 탭, 레이블, 스크롤 및 대화 제목의 일반 설정" width="100%">
</p>
...
| `.grimoire/logs/YYYY-MM-DD.jsonl` | 선택적으로 활성화하는 민감 정보가 제거된 디버그 로그 |
